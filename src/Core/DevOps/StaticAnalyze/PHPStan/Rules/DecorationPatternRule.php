...
use HeyPanel\Core\Framework\Store\Services\AbstractExtensionLifecycle;
use HeyPanel\Core\Framework\Store\Services\AbstractExtensionStoreLicensesService;
use PhpParser\Node;
...
class DecorationPatternRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope): array
    {
        ...
        // only validate classes that extend from abstract HeyPanel classes
        if (!$parent || !\str_starts_with($parent->getName(), 'HeyPanel\\') || !$parent->isAbstract()) {
            return [];
        }
        ...
        if ($this->isInternal($doc)) {
            $errors[] = RuleErrorBuilder::message('Decoration error: Concrete class is marked as @internal. Remove `getDecorated` (if not intended that these classes can be decorated) or remove @internal annotation')
                ->identifier('heypanel.decorationPattern')
                ->build();
        }
