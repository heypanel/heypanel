...
namespace HeyPanel\Core\DevOps\StaticAnalyze\PHPStan\Rules\Internal;

use HeyPanel\Core\Framework\Bundle;
use HeyPanel\Core\Framework\DataAbstractionLayer\Command\RefreshIndexCommand;
...
class InternalClassRule implements Rule
{
        ...
        if ($this->isTestClass($node)) {
            return [
                RuleErrorBuilder::message(\sprintf(
                    ...
                    ->identifier('heypanel.internalClass')
                    ->build(),
            ];
        }
    ...
    private function isTestClass(InClassNode $node): bool
    {
        ...
        if (\str_contains($namespace, 'HeyPanel\\Core\\Test\\Stub\\')) {
            return false;
        }
    ...
    private function isExample(InClassNode $node): bool
    {
        ...
        return \str_contains($namespace, 'HeyPanel\\Tests\\Examples\\');
    }
}
