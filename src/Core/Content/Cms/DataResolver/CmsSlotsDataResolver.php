...
use HeyPanel\Core\Content\Cms\Aggregate\CmsSlot\CmsSlotCollection;
...
use HeyPanel\Core\Content\Cms\Extension\CmsSlotsDataCollectExtension;
use HeyPanel\Core\Content\Cms\Extension\CmsSlotsDataEnrichExtension;
use HeyPanel\Core\Content\Cms\Extension\CmsSlotsDataResolveExtension;
...
class CmsSlotsDataResolver
{
     ...
     * @param array<string, ChannelRepository> $repositories
     */
    ...
    private function __resolve(CmsSlotCollection $slots, ResolverContext $resolverContext): CmsSlotCollection
    {
        ...
        $identifierResult = $this->fetchByIdentifier($directReads, $resolverContext->getChannelContext());
        $criteriaResult = $this->fetchByCriteria($searches, $resolverContext->getChannelContext());
     ...
     */
    private function fetchByIdentifier(array $directReads, ChannelContext $context): array
    {
        $entities = [];
        ...
        foreach ($directReads as $definitionClass => $ids) {
            ...
            $repository = $this->getChannelApiRepository($definition);
     ...
     */
    private function fetchByCriteria(array $searches, ChannelContext $context): array
    {
        $searchResults = [];
    ...
    private function getChannelApiRepository(EntityDefinition $definition): ?ChannelRepository
    {
