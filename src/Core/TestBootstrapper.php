...
use HeyPanel\Core\DevOps\StaticAnalyze\StaticAnalyzeKernel;
use HeyPanel\Core\Framework\Adapter\Kernel\KernelFactory;
use HeyPanel\Core\Framework\Plugin\KernelPluginLoader\DbalKernelPluginLoader;
use HeyPanel\Core\Framework\Test\TestCaseBase\KernelLifecycleManager;
...
class TestBootstrapper
{
    ...
    public function bootstrap(): TestBootstrapper
    {
        ...
        if ($this->commercialEnabled && $this->getPluginPath('HeyCart')) {
            $this->addActivePlugins('HeyCart');
        }
    ...
    public function addCallingPlugin(?string $pathToComposerJson = null): TestBootstrapper
    {
        ...
        $baseClass = $composer['extra']['heypanel-plugin-class'] ?? '';
        if ($baseClass === '') {
            throw new \RuntimeException('composer.json does not contain `extra.heypanel-plugin-class`. Path: ' . $pathToComposerJson);
        }
