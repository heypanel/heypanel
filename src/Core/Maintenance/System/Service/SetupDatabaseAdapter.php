...
class SetupDatabaseAdapter
{
    ...
    public function initializeHeyPanelDb(Connection $connection, ?string $database = null): bool
    {
        if (!$hasHeyPanelTables = $this->hasHeyPanelTables($connection, $database)) {
            $connection->executeStatement($this->getBaseSchema());
        }

        return !$hasHeyPanelTables;
    }

    public function hasHeyPanelTables(Connection $connection, ?string $database = null): bool
    {
        if ($database) {
