...
class WebsiteConfigurator
{
    ...
    {
        if ($shopName) {
            $this->setSystemConfig('core.basicInformation.websiteName', $shopName);
        }
    ...
    public function setDefaultLanguage(string $locale): void
    {
        ...
        if (!$currentLocale) {
            throw MaintenanceException::websiteConfigurationNotValid('Default language locale not found');
        }
        ...
        if ($locale === 'en-GB' && $currentLocale['code'] === 'zh-CN') {
    ...
    public function setDefaultCurrency(string $currencyCode): void
    {
        ...
        if (!$currentCurrencyIso) {
            throw MaintenanceException::websiteConfigurationNotValid('Default currency not found');
        }
    ...
    private function createNewCurrency(string $currencyCode): string
    {
        ...
        if (!Currencies::exists($currencyCode)) {
            throw MaintenanceException::websiteConfigurationNotValid(\sprintf('Currency with iso code "%s" not found', $currencyCode));
        }
