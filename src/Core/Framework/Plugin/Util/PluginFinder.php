...
use HeyPanel\Core\Framework\Plugin\Composer\Factory;
use HeyPanel\Core\Framework\Plugin\Composer\PackageProvider;
use HeyPanel\Core\Framework\Plugin\Exception\ExceptionCollection;
use HeyPanel\Core\Framework\Plugin\Exception\PluginComposerJsonInvalidException;
use HeyPanel\Core\Framework\Plugin\Struct\PluginFromFileSystemStruct;
use Symfony\Component\Finder\Exception\DirectoryNotFoundException;
use Symfony\Component\Finder\Finder;

class PluginFinder
{
    final public const COMPOSER_TYPE = 'heypanel-platform-plugin';
    private const SHOPWARE_PLUGIN_CLASS_EXTRA_IDENTIFIER = 'heypanel-plugin-class';
    ...
    private function loadLocalPlugins(string $pluginDir, IOInterface $composerIO, ExceptionCollection $errors): array
    {
        ...
        try {
            ...
            foreach ($filesystemPlugins as $filesystemPlugin) {
                ...
                if (!$this->isHeyPanelPluginType($package) || !$this->isPluginComposerValid($package)) {
    ...
    private function isHeyPanelPluginType(CompletePackageInterface $package): bool
    {
    ...
    private function enrichWithVendorPlugins(
        ...
        foreach ($composerPackages as $composerPackage) {
            if (!$this->isHeyPanelPluginType($composerPackage)) {
                continue;
            }
        ...
        if ($this->isHeyPanelPluginType($root) && $this->isPluginComposerValid($root)) {
