...
class PluginService
{
    ...
    public function refreshPlugins(Context $heypanelContext, IOInterface $composerIO): ExceptionCollection
    {
        ...
        $installedPlugins = $this->getPlugins(new Criteria(), $heypanelContext);
        ...
        foreach ($pluginsFromFileSystem as $pluginFromFileSystem) {
            ...
            $pluginData['translations'] = $this->getTranslations($heypanelContext, $extra);
        ...
        if ($plugins !== []) {
            foreach ($plugins as $plugin) {
                try {
                    $this->pluginRepo->upsert([$plugin], $heypanelContext);
                } catch (HeyPanelHttpException $exception) {
        ...
        if (\count($deletePluginIds) !== 0) {
            ...
            $this->pluginRepo->delete($deletePlugins, $heypanelContext);
        }
    ...
    private function getTranslations(Context $context, array $extra): array
    {
         ...
         * @example payload
         * {
         *     "heypanel-plugin-class":"Swag\\MyDemoData\\MyDemoData",
         *     "label":{
