...
abstract class Plugin extends Bundle
{
     ...
     * If needed, return true and HeyPanel will execute `composer require` during the plugin installation.
     * When the plugins gets uninstalled, HeyPanel executes `composer remove`
     */
    ...
    {
        // namespace should not start with `heypanel`
        if (str_starts_with(mb_strtolower($this->getMigrationNamespace()), 'heypanel') && !str_starts_with(mb_strtolower($this->getMigrationNamespace()), 'heypanel\commercial')) {
            throw PluginException::cannotDeleteHeyPanelMigrations();
        }
