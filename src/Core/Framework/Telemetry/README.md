# Telemetry
This component contains the code for the collection of telemetry in heypanel applications.
...
In the HeyPanel application, various types of metrics can be collected:
...
To create a new transport, you need to implement the `HeyPanel\Core\Framework\Telemetry\Metrics\MetricTransportInterface` interface.

Transports should support all basic metric types. If a transport does not support a specific metric type, it should throw a `HeyPanel\Core\Framework\Telemetry\Metrics\Exception\MetricNotSupportedException` exception.
...
use HeyPanel\Core\Framework\Telemetry\Metrics\MetricTransportInterface;use HeyPanel\Core\Framework\Telemetry\TelemetryException;
...
<service id="YourPackage/NameSpace/MetricTransportImplementation">
    <tag name="heypanel.metric_transport"/>
</service>
```

Real-world example of transport can be found in the http://github.com/heypanel/opentelemetry/ package.

### Usage

There are two recommended ways to emit metrics in the HeyPanel application:

#### Using the Events system


##### Attaching to Events
You can attach metrics to the events using the predefined attributes from the `HeyPanel\Core\Framework\Telemetry\Metrics\Attribute` namespace.
...
#[\HeyPanel\Core\Framework\Telemetry\Metrics\Attribute\Counter(name="my_metric", value: 1, description: "My metric description")]
class MyEvent
{
}
```
...
```php
use HeyPanel\Core\Framework\Telemetry\Metrics\Attribute\Counter;
...
namespace HeyPanel\Package\Subscriber\Statistics;
...
```php
\HeyPanel\Core\Framework\Telemetry\Metrics\MeterProvider::meter()->emit(new \HeyPanel\Core\Framework\Telemetry\Metrics\Metric\Counter(name: 'my_metric', value: 1,  description:  'My metric description'));
```
...
- The **Histogram** metric does not have explicit support for buckets (like `public ?array $buckets = null`). The reason
  ...
  make it possible to configure the buckets depending on the heypanel installation.  
