...
use HeyPanel\Core\Framework\Adapter\Filesystem\PrefixFilesystem;
use HeyPanel\Core\Framework\DependencyInjection\CompilerPass\BusinessEventRegisterCompilerPass;
...
abstract class Bundle extends SymfonyBundle
{
    ...
    /**
     * Returns a list of all action event class references of this bundle. The events will be registered inside the `\HeyPanel\Core\Framework\Event\BusinessEventRegistry`.
     *
    ...
    protected function registerMigrationPath(ContainerBuilder $container): void
    {
        ...
        $container->register(MigrationSource::class . '_' . $this->getName(), MigrationSource::class)
            ...
            ->addTag('heypanel.migration_source');
    }
    ...
    private function registerFilesystem(ContainerBuilder $container, string $key): void
    {
        ...
        $parameterKey = \sprintf('heypanel.filesystem.%s', $key);
