...
namespace HeyPanel\Core\Framework\Adapter\Command;
...
/**
 * @phpstan-import-type RedisTypeHint from \HeyPanel\Core\Framework\Adapter\Cache\RedisConnectionFactory
 */
...
class CacheWatchDelayedCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$this->container->has('heypanel.cache.invalidator.storage.redis_adapter')) {
        ...
        $adapter = $this->container->get('heypanel.cache.invalidator.storage.redis_adapter');

        if (method_exists($adapter, 'sCustomers') === false) {
            $output->writeln('Redis adapter does not support sCustomers method.');

            return self::FAILURE;
        }

        $before = $adapter
            ->sCustomers('invalidation');
