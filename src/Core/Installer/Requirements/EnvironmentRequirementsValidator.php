...
use HeyPanel\Core\Installer\Requirements\Struct\RequirementCheck;
use HeyPanel\Core\Installer\Requirements\Struct\RequirementsCheckCollection;
use HeyPanel\Core\Installer\Requirements\Struct\SystemCheck;
...
class EnvironmentRequirementsValidator implements RequirementsValidatorInterface
{
    ...
    {
        $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('heypanel/platform', '*');
        if (!$platform) {
            $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('heypanel/core', '*');
        }
        if (!$platform) {
