 ...
 * It's only used for HeyAdmin plugins. This solves the problem of having multiple Vue instances in the same project.
 *
 * Inspired by: https://github.com/crcong/vite-plugin-externals/
 */
export default function viteExternalsPlugin(): Plugin {
    return {
        name: 'heyadmin-vite-plugin-vue-globals',
        ...
        async config(config) {
            ...
            const cachePath = path.join(process.cwd(), 'node_modules', '.heyadmin-vite-plugin-vue-globals');
            ...
            await writeFile(vueJsCachePath, `module.exports = window['HeyAdmin']?.['Vue']`);
