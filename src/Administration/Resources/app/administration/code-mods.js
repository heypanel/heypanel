...
// Available HeyPanel versions
const heypanelVersions = [
    '6.6',
    '6.7',
];
...
const optionDefinitions = [
    ...
    {
        description: 'HeyPanel root. Default ../../../../../',
        name: 'heypanel-root',
        alias: 'r',
        type: String,
    },
    ...
    },
    {
        ...
        description: `Define the HeyPanel version for loading the correct codemods. Available: ${heypanelVersions.join(', ')}`,
        name: 'heypanel-version',
        alias: 'v',
        type: String,
    },
];
...
const sections = [
    {
        header: 'HeyPanel Admin code mods',
        content: 'Run heypanel code mods in your plugin!',
    },
    {
        header: 'Synopsis',
        content: [
            '{bold Run as npm script inside <heypanelRoot>/src/Administration/Resources/app/administration}:',
            '$ npm run code-mods -- [{bold --fix}] {bold --plugin-name} {underline SwagExamplePlugin}',
            '$ npm run code-mods -- {bold --help}',
            '',
            '{bold Run as composer script inside <heypanelRoot>}:',
...
(async function codeMods() {
    ...
    const heypanelVersion = options['heypanel-version'];
    if (heypanelVersion && !heypanelVersions.includes(heypanelVersion)) {
        console.error(colors.red('Invalid HeyPanel version. Available: 6.6, 6.7'));
        process.exit(1);
    }
    ...
    if (options['heypanel-root']) {
        let optionsRoot = options['heypanel-root'];
    ...
    for (const srcDir of srcDirectories) {
        ...
        try {
            await lintFiles([workingDir], fix, heypanelVersion);
...
async function lintAndFix(eslint, filePaths, heypanelVersion) {
    const results = await eslint.lintFiles(filePaths, heypanelVersion);
...
async function lintFiles(filePaths, fix, heypanelVersion) {
        ...
        overrides: [
            {
                ...
                rules: {
                    ...(() => {
                        if (isVersionNewerOrSame(heypanelVersion, '6.7')) {
                            return {
            ...
            },
            {
                extends: [
                    ...
                    '@heypanel-ag/eslint-config-base',
                ],
                files: ['**/*.js'],
                excludedFiles: [
                    '*.spec.js',
    ...
    const results = await lintAndFix(eslint, filePaths, heypanelVersion);
