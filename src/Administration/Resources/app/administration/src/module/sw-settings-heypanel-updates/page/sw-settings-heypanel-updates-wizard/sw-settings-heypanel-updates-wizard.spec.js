...
describe('module/sw-settings-heypanel-updates/page/sw-settings-heypanel-updates-wizard', () => {
    let wrapper;

    beforeEach(async () => {
        HeyPanel.Application.view.deleteReactive = () => {};
        wrapper = mount(
            await wrapTestComponent('sw-settings-heypanel-updates-wizard', {
                sync: true,
            }),
            {
                global: {
                        ...
                        updateService: {
                            ...
                            checkRequirements: () =>
                                Promise.resolve([
                                    ...
                                    {
                                        name: 'validHeyPanelLicense',
                                        result: false,
                                        message: 'validHeyPanelLicense',
                                        vars: [],
                                        extensions: [],
                                    },
                                ]),
                    ...
                    stubs: {
                        ...
                        'sw-settings-heypanel-updates-requirements': await wrapTestComponent(
                            'sw-settings-heypanel-updates-requirements',
                        ),
                        'sw-data-grid': await wrapTestComponent('sw-data-grid'),
                        'sw-card-view': await wrapTestComponent('sw-card-view'),
                        'sw-ignore-class': true,
                        'sw-settings-heypanel-updates-info': {
                            template: '<div></div>',
                        },
                        'sw-settings-heypanel-updates-plugins': {
                            template: '<div></div>',
                        },
                        'sw-loader': {
                            template: '<div></div>',
    ...
    it('should disable the button if one requirement is not met', async () => {
        const button = wrapper.findByText('button', 'sw-settings-heypanel-updates.infos.startUpdate');
    ...
    it('deactivate plugins success', async () => {
        ...
        expect(redirectSpy).toHaveBeenCalledWith(`${HeyPanel.Context.api.basePath}/heypanel-installer.phar.php`);
    });
    ...
    it('click on update button', async () => {
        ...
        await wrapper.get('.sw-settings-heypanel-updates-wizard__start-update').trigger('click');
        await flushPromises();

        expect(wrapper.vm.updateModalShown).toBe(true);

        expect(wrapper.find('.sw-settings-heypanel-updates-check__start-update').exists()).toBe(true);

        await wrapper.get('.sw-settings-heypanel-updates-check__start-update-backup-checkbox input').setChecked(true);

        const redirectSpy = jest.fn();
        wrapper.vm.redirectToPage = redirectSpy;

        await wrapper.get('.sw-settings-heypanel-updates-check__start-update-button').trigger('click');
