import template from './sw-settings-heypanel-updates-wizard.html.twig';
import './sw-settings-heypanel-updates-wizard.scss';
...
export default Component.wrapComponentConfig({
    ...
    computed: {
        ...
        updateButtonTooltip() {
            ...
            return {
                message: this.$tc('sw-settings-heypanel-updates.infos.requirementsNotMet'),
                position: 'bottom',
            };
        },
        ...
        optionDeactivateIncompatibleTranslation() {
            const deactivateIncompatTrans = this.$tc('sw-settings-heypanel-updates.plugins.actions.deactivateIncompatible');
            const isRecommended =
                this.displayIncompatiblePluginsWarning && !this.displayUnknownPluginsWarning
                    ? this.$tc('sw-settings-heypanel-updates.plugins.actions.recommended')
                    : '';
        ...
        optionDeactivateAllTranslation() {
            const deactiveAllTrans = this.$tc('sw-settings-heypanel-updates.plugins.actions.deactivateAll');
            const isRecommended =
                this.displayIncompatiblePluginsWarning && this.displayUnknownPluginsWarning
                    ? this.$tc('sw-settings-heypanel-updates.plugins.actions.recommended')
                    : '';
    ...
    methods: {
        ...
        startUpdateProcess() {
            ...
            this.createNotificationSuccess({
                message: this.$tc('sw-settings-heypanel-updates.notifications.updateStarted'),
            });
        ...
        stopUpdateProcess() {
            ...
            this.createNotificationInfo({
                message: this.$tc('sw-settings-heypanel-updates.notifications.updateStopped'),
            });
        },
            ...
            this.updateService
                ...
                .catch(() => {
                    this.createNotificationError({
                        message: this.$tc('sw-settings-heypanel-updates.notifications.downloadFailed'),
                    });
                });
        },
            ...
            this.updateService
                ...
                .then((response) => {
                    ...
                    if (response.offset === response.total) {
                        this.redirectToPage(`${HeyPanel.Context.api.basePath}/heypanel-installer.phar.php`);
                    } else {
                        this.deactivatePlugins(response.offset);
                    }
                })
                .catch((e: HeyPanelApiError) => {
                    ...
                    if (context.code === 'FRAMEWORK__PLUGIN_HAS_DEPENDANTS') {
                        ...
                        this.createNotificationError({
                            message: this.$tc('sw-settings-heypanel-updates.notifications.deactivationFailed'),
                        });
                    }
                });
        },
