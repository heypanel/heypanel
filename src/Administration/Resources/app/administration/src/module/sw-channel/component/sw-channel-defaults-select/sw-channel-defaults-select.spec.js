...
async function createWrapper(customProps = {}) {
    const channel = {};
    channel.getEntityName = () => '';

    return shallowMount(
        await wrapTestComponent('sw-channel-defaults-select', {
            sync: true,
        }),
        {
            global: {
...
describe('src/module/sw-channel/component/sw-channel-defaults-select', () => {
    it('should have selects enabled', async () => {
        const wrapper = await createWrapper();

        const multiSelect = wrapper.getComponent('.sw-channel-detail__select-countries');
        const singleSelect = wrapper.getComponent('.sw-channel-detail__assign-countries');
