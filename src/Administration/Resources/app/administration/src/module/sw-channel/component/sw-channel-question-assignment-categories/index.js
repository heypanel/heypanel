...
import template from './sw-channel-question-assignment-categories.html.twig';
import './sw-channel-question-assignment-categories.scss';
...
export default {
        ...
        categoriesCollection: {
            handler(value) {
                ...
                this.getProductFromCategories(value.getIds())
                    .then((questions) => {
                        this.$emit('selection-change', questions, 'categoryProducts');
                    })
                    .catch((error) => {
    ...
    methods: {
        ...
        searchCategories(term) {
            return this.categoryRepository.search(this.categorySearchCriteria(term), HeyPanel.Context.api);
        },
        ...
        questionCriteria(categories) {
            const questionCriteria = new Criteria(1, 500);
            questionCriteria.addFilter(
                Criteria.multi('AND', [
        ...
        getProductFromCategories(categories) {
            return this.questionRepository.search(this.questionCriteria(categories), HeyPanel.Context.api);
        },
    },
};
