...
export default {
    ...
    computed: {
        currentUserId() {
            return HeyPanel.Store.get('session').currentUser.id;
        },
    ...
    methods: {
        ...
        loadUserConfig() {
            return this.userConfigRepository.search(this.userConfigCriteria, HeyPanel.Context.api).then((userConfigs) => {
                if (userConfigs.length === 0) {
                    this.userConfig = this.userConfigRepository.create(HeyPanel.Context.api);
        ...
        onSave() {
            ...
            return this.countryRepository
                .save(this.country, HeyPanel.Context.api)
                .then(() => {
                    if (userConfigValue && Object.keys(userConfigValue).length > 0) {
                        this.userConfigRepository.save(this.userConfig, HeyPanel.Context.api).then(() => {
                            this.loadUserConfig();
                        });
                    }
                    this.loadEntityData();
        ...
        onUpdateCountry(path, value) {
            HeyPanel.Utils.object.set(this.country, path, value);
        },
    },
};
