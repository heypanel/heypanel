...
export default {
    ...
    data() {
        return {
            ...
            availableVariables: {},
            entitySchema: Object.fromEntries(HeyPanel.EntityDefinition.getDefinitionRegistry()),
    ...
    methods: {
        ...
        loadEntityData() {
            ...
            this.isLoading = true;
            this.mailTemplateRepository.get(this.mailTemplateId, HeyPanel.Context.api, criteria).then((item) => {
        ...
        createMediaCollection() {
            return new EntityCollection('/media', 'media', HeyPanel.Context.api);
        },
        ...
        getMailTemplateMedia() {
            ...
            this.mailTemplate.media.forEach((mediaAssoc) => {
                if (mediaAssoc.languageId === HeyPanel.Context.api.languageId) {
        ...
        onClickTestMailTemplate() {
            ...
            this.channelRepository.get(this.testMailChannelId, Context.api, criteria).then((channel) => {
                if (!channel.languages.has(HeyPanel.Context.api.languageId)) {
        ...
        createMailTemplateMediaAssoc(mediaItem) {
            ...
            mailTemplateMedia.languageId = HeyPanel.Context.api.languageId;
        ...
        _checkIfMediaIsAlreadyUsed(mediaId) {
            return this.mailTemplate.media.some((mailTemplateMedia) => {
                return (
                    mailTemplateMedia.mediaId === mediaId && mailTemplateMedia.languageId === HeyPanel.Context.api.languageId
                );
            });
        },
        ...
        loadAvailableVariables(variable, variableEntitySchema) {
            ...
            const foundVariables = Object.keys(HeyPanel.Utils.get(this.mailTemplateType.templateData, variable));

            const keys = foundVariables.map((val) => {
                const availableVariable = HeyPanel.Utils.get(this.mailTemplateType.templateData, variablePath.concat(val));
        ...
        isToManyAssociationVariable(variable) {
            ...
            const field = HeyPanel.Utils.get(this.entitySchema, `${variables.join('.')}`);
        ...
        loadInitialAvailableVariables() {
            ...
            Object.keys(this.mailTemplateType.templateData).forEach((variable) => {
                const availableVariable = HeyPanel.Utils.get(this.mailTemplateType.templateData, variable);
                let length = 0;
