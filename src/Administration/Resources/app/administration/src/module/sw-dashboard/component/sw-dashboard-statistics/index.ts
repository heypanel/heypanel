 ...
 *
 * @private might get removed with any update (even minor!) as it likely gets replaced by heypanel analytics
 */
export default HeyPanel.Component.wrapComponentConfig({
    ...
    computed: {
            ...
            return {
                ...
                yaxis: {
                    ...
                    labels: {
                        ...
                        formatter: (value: string) =>
                            HeyPanel.Utils.format.currency(
                                Number.parseFloat(value),
                                HeyPanel.Context.app.systemCurrencyISOCode as string,
                                2,
                            ),
                    },
                },
        ...
        today() {
            const today = HeyPanel.Utils.format.dateWithUserTimezone();
        ...
        systemCurrencyISOCode() {
            return HeyPanel.Context.app.systemCurrencyISOCode;
        },

        isSessionLoaded() {
            return !HeyPanel.Store.get('session')?.userPending;
        },
    ...
    methods: {
        ...
        fetchHistory(paid: boolean, since: string) {
            ...
            const timezone = HeyPanel.Store.get('session').currentUser?.timeZone ?? 'UTC';
        ...
        formatDateToISO(date: Date) {
            return HeyPanel.Utils.format.toISODate(date, false);
        },

        formatChartHeadlineDate(date: Date) {
            const lastKnownLang = HeyPanel.Application.getContainer('factory').locale.getLastKnownLocale();
        ...
        getDateAgo(range: HistoryDateRange): Date {
            const date = HeyPanel.Utils.format.dateWithUserTimezone();
