    ...
    return mount(
        ...
        }),
        {
            ...
            global: {
                ...
                provide: {
                    cmsService: HeyPanel.Service('cmsService'),
                    cmsElementFavorites: HeyPanel.Service('cmsElementFavorites'),
                },
            },
        },
    );
...
describe('module/sw-cms/component/sw-cms-slot', () => {
    beforeAll(async () => {
        ...
        const cmsService = HeyPanel.Service('cmsService');
        ...
        const buildConfig = (name, parameters = {}) => {
            return {
                ...
                flag: true,
                allowedPageTypes: ['question_list'],
                ...parameters,
            };
        };

        cmsService.registerCmsElement(buildConfig('question_list_slot'));
        cmsService.registerCmsElement(
            buildConfig('question_detail_slot', {
                allowedPageTypes: ['question_detail'],
            }),
        );
        cmsService.registerCmsElement(
            buildConfig('question_detail_slot2', {
                allowedPageTypes: ['question_detail'],
            }),
        );
        cmsService.registerCmsElement(
            buildConfig('question_detail_slot3', {
                allowedPageTypes: ['question_detail'],
            }),
        );
        cmsService.registerCmsElement(
    ...
    beforeEach(() => {
        ...
        store.currentPageType = 'question_list';

        const cmsElementFavorites = HeyPanel.Service('cmsElementFavorites');
    ...
    it('should collect grouped CMS elements, depending on favorites', async () => {
        const store = HeyPanel.Store.get('cmsPage');
        store.currentPageType = 'question_detail';

        const wrapper = await createWrapper();
        wrapper.vm.cmsElementFavorites.update(true, 'question_detail_slot');
        wrapper.vm.cmsElementFavorites.update(true, 'question_detail_slot2');
        ...
        expect(actualFavorites.items[0].name).toBe('question_detail_slot');
        expect(actualFavorites.items[1].name).toBe('question_detail_slot2');
        ...
        expect(actualNonFavorites.items[0].name).toBe('question_detail_slot3');
    });
    ...
    it('should group elements correctly depending on being a favorite or not', async () => {
        ...
        expect(wrapper.vm.elementInElementGroup({ name: 'question_detail_slot' }, 'favorite')).toBe(false);

        wrapper.vm.cmsElementFavorites.update(true, 'question_detail_slot');
        expect(wrapper.vm.elementInElementGroup({ name: 'question_detail_slot' }, 'favorite')).toBe(true);
    });
    ...
    it('should not group elements, when they are not favorites', async () => {
        ...
        expect(wrapper.vm.elementInElementGroup({ name: 'question_detail_slot' }, 'whatever')).toBe(true);
    });
});
