...
const cmsElements = {
    text: {
        collect(slot) {
            if (slot.config.questions?.entity) {
                return {
                    'entity-questions': {
                        value: [
            ...
            if (slot.config.categories?.entity) {
                ...
                switch (debugProperty) {
                    case 'associations':
                        searchCriteria.addAssociation('question');
                        break;
                    case 'filters':
...
HeyPanel.Service().register('cmsService', () => {
    return {
...
function getService() {
    return HeyPanel.Service().get('cmsDataResolverService');
}
...
describe('module/sw-cms/service/cmsDataResolver.service.js', () => {
    ...
    it('should enrich cmsElements if entities are attached', async () => {
        ...
        expect(Object.keys(cmsElements.enrichAssertHelper[0].slotEntities)).toContain('entity-questions');
    });
