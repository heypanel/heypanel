...
async function createWrapper(layoutType = 'question_list', systemConfigApiServiceOverrides = {}) {
    return mount(
        ...
        }),
        {
            ...
            props: {
                page: {
                    categories: new EntityCollection(null, null, HeyPanel.Context.api, new Criteria(1, 25), mockCategories),
                    questions: new EntityCollection(null, null, HeyPanel.Context.api, new Criteria(1, 25), mockProducts),
            ...
            global: {
                stubs: {
                    ...
                    'sw-loader': true,
                    'sw-cms-question-assignment': {
                        template: `
                        <div class="sw-cms-question-assignment">
                            ...
                            <slot
                                ...
                                <img
                                    ...
                                    alt=""
                                >
                                <p>{{ $tc('sw-cms.components.cmsLayoutAssignmentModal.questions.questionAssignmentEmptyStateDescription') }}</p>
                            </slot>
                        </div>
                    `,
                    },
...
describe('module/sw-cms/component/sw-cms-layout-assignment-modal', () => {
    ...
    it('should show an empty state, when question_detail page has no questions', async () => {
        global.activeAclRoles = ['system.system_config'];

        const wrapper = await createWrapper('question_detail');
        await wrapper.setProps({
            questions: new EntityCollection('/questions', 'questions', null, new Criteria(1, 25), mockProducts),
        });
        await flushPromises();

        expect(wrapper.find('.sw-cms-question-assignment__empty-state').exists()).toBeTruthy();
    });
    ...
    it('should load system config with different channel', async () => {
        global.activeAclRoles = ['system.system_config'];
    ...
    it('should load system config with different channel without matching shop pages', async () => {
        global.activeAclRoles = ['system.system_config'];
    ...
    it('should load system config when changing channel', async () => {
        global.activeAclRoles = ['system.system_config'];
    ...
    it('should render question selection', async () => {
        const wrapper = await createWrapper('question_detail');

        expect(wrapper.find('.sw-cms-layout-assignment-modal__question-select').exists()).toBeTruthy();
    });

    it('should store previous questions on component creation', async () => {
        const wrapper = await createWrapper('question_detail');
    ...
    it('should add questions', async () => {
        ...
        await wrapper.setProps({
            page: {
                ...wrapper.vm.page,
                questions: new EntityCollection(null, null, null, new Criteria(1, 25), [
                    ...mockProducts,
                    {
                        name: 'New question',
                        id: 'uuid4',
        ...
        expect(wrapper.vm.page.questions).toEqual(
            expect.arrayContaining([
                {
                    name: 'New question',
                    id: 'uuid4',
    ...
    it('should add a question which already has a different assigned layout', async () => {
        const wrapper = await createWrapper('question_detail');
        ...
        expect(wrapper.find('.sw-cms-layout-assignment-modal__confirm-changes-modal').exists()).toBeTruthy();
        expect(
            wrapper.find('.sw-cms-layout-assignment-modal__confirm-text-questions-assigned-layouts').exists(),
        ).toBeTruthy();
    ...
    it('should remove questions and save the changes', async () => {
        const wrapper = await createWrapper('question_detail');
        ...
        // Change warning should appear because of removed category
        expect(wrapper.find('.sw-cms-layout-assignment-modal__confirm-changes-modal').exists()).toBeTruthy();
        expect(wrapper.find('.sw-cms-layout-assignment-modal__confirm-text-questions').exists()).toBeTruthy();
    ...
    it('should remove questions but discard changes', async () => {
        const wrapper = await createWrapper('question_detail');
        ...
        // Verify categories are restored to initial categories
        expect(wrapper.vm.page.questions).toEqual(expect.arrayContaining(mockProducts));
        expect(wrapper.emitted('modal-close')).toStrictEqual([[false]]);
    });

    it('should remove questions but keep editing', async () => {
        const wrapper = await createWrapper('question_detail');
