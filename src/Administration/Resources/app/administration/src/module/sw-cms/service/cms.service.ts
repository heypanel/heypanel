...
import type Criteria from '@heypanel-ag/meteor-admin-sdk/es/data/Criteria';
...
class CmsService {
    ...
    public registerCmsElement(config: CmsElementConfig): boolean {
        ...
        HeyPanel.Application.view?.setReactive(this.elementRegistry, config.name, config);
    ...
    public getEntityMappingTypes(entityName: string) {
        const schema = HeyPanel.EntityDefinition.has(entityName) ? HeyPanel.EntityDefinition.get(entityName) : undefined;
    ...
    private async addCustomFieldsToMappingTypes(entityName: string, mappings: EntityMappings): Promise<void> {
        const customFieldRepository = HeyPanel.Service('repositoryFactory').create('custom_field');
        const criteria = new HeyPanel.Data.Criteria(1, 50);
        criteria.addFilter(HeyPanel.Data.Criteria.equals('customFieldSet.relations.entityName', entityName));
        criteria.addFilter(HeyPanel.Data.Criteria.equals('active', 1));
        criteria.addFilter(
            HeyPanel.Data.Criteria.multi('OR', [
                HeyPanel.Data.Criteria.equals('type', 'text'),
                HeyPanel.Data.Criteria.equals('type', 'datetime'),
                HeyPanel.Data.Criteria.equals('type', 'html'),
            ]),
        );

        const customFields = await customFieldRepository.search(criteria, HeyPanel.Context.api);
    ...
    private handleAssociationMapping(
        ...
        if (toOneAssociation && propSchema.entity) {
            ...
            if (deep) {
                const schema = HeyPanel.EntityDefinition.get(propSchema.entity);
    ...
    static getEntityData(slot: RuntimeSlot, configKey: string): CmsSlotData {
        ...
        const entityData: CmsSlotData = {
            value: [],
            key: configKey,
            searchCriteria: new HeyPanel.Data.Criteria(1, 25),
            ...entity,
        };
        ...
        entityData.searchCriteria = entity?.criteria ? entity.criteria : new HeyPanel.Data.Criteria(1, 25);
    ...
    Object.keys(slot.config).forEach((configKey) => {
        ...
        if (entity && value) {
            ...
            const entityData: CmsSlotData = {
                value: [value].flat(),
                key: configKey,
                searchCriteria: entity.criteria ? entity.criteria : new HeyPanel.Data.Criteria(1, 25),
                ...entity,
            };
