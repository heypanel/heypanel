...
const { Component, Mixin, Utils } = HeyPanel;
const { mapPropertyErrors } = Component.getComponentHelper();
const { HeyPanelError } = HeyPanel.Classes;
const { debounce } = HeyPanel.Utils;
const { cloneDeep, getObjectDiff } = HeyPanel.Utils.object;
...
export default {
    ...
    data() {
        return {
            ...
            questionDetailBlocks: [
                ...
                {
                    type: 'question-description-reviews',
                    elements: [
                        {
                            slot: 'content',
                            type: 'question-description-reviews',
    ...
    methods: {
        createdComponent() {
            ...
            const isSystemDefaultLanguage = HeyPanel.Store.get('context').isSystemDefaultLanguage;
        ...
        loadPage(pageId) {
            ...
            return this.pageRepository
                .get(pageId, HeyPanel.Context.api, this.loadPageCriteria)
                .then((page) => {
                    this.page = { sections: [] };
                    this.page = page;
        ...
        async loadDemoProduct(entityId) {
            ...
            const context = { ...HeyPanel.Context.api, inheritance: true };

            const response = await this.questionRepository.search(criteria, context);
        ...
        onSaveEntity() {
            ...
            return this.pageRepository
                .save(this.page, HeyPanel.Context.api, false)
                .then(() => {
                    this.isLoading = false;
        ...
        addError({ property = null, payload = {}, code = CMS.REQUIRED_FIELD_ERROR_CODE, message = '' } = {}) {
            ...
            const error = new HeyPanelError({
                code,
                detail: message,
                meta: { parameters: payload },
            });

            HeyPanel.Store.get('error').addApiError({ expression, error });
        },

        getError(property) {
            return HeyPanel.Store.get('error').getApiError(this.page, property);
        },
        ...
        pageIsValid() {
            ...
            HeyPanel.Store.get('error').resetApiErrors();
        ...
        listingPageValidation() {
            ...
            const foundListingBlock = this.page.sections.some((section) => {
                return section.blocks.some((block) => {
                    return block.type === 'question-listing';
                });
            });

            if (foundListingBlock) {
                this.cmsBlocks['question-listing'].hidden = true;
                return true;
            }
            ...
            this.cmsBlocks['question-listing'].hidden = false;
        ...
        onPageTypeChange(pageType) {
            ...
            if (this.page.type === CMS.PAGE_TYPES.LISTING) {
                ...
                this.page.sections.forEach((section) => {
                    section.blocks.forEach((block) => {
                        if (block.type === 'question-listing') {
            ...
            const listingElements = [
                {
                    ...
                    slot: 'content',
                    type: 'question-listing',
                    config: {},
                },
            ];

            this.processBlock(listingBlock, 'question-listing');
            this.processElements(listingBlock, listingElements);
        },

        processProductDetailType() {
            this.questionDetailBlocks.forEach((block) => {
        ...
        processBlock(block, blockType) {
            ...
            if (this.isProductPage && defaultConfig) {
                defaultConfig = {
                    ...
                    marginLeft: '0',
                    marginRight: '0',
                    marginTop: blockType === 'gallery-buybox' || blockType === 'question-description-reviews' ? '20px' : '0',
                    marginBottom:
                        blockType === 'question-heading' || blockType === 'question-description-reviews' ? '20px' : '0',
                };
            }
        ...
        async onConfirmLayoutSetAsDefault() {
            ...
            if (this.page.type === 'question_detail') {
                configKey = 'question_cms_page';
            }
        ...
        async setDefaultLayout() {
            ...
            const questionDetailId = response['core.cms.default_category_cms_page'];
            const questionListId = response['core.cms.default_question_cms_page'];
            const isLiveVersion = this.page.versionId === HeyPanel.Context.api.liveVersionId;
