...
const searchMock = jest.fn(() => Promise.resolve(questionMocks));
...
describe('module/sw-cms/component/sw-cms-layout-modal', () => {
                ...
                filters: [
                    {
                        type: 'equalsAny',
                        field: 'type',
                        value: 'page|landingpage|question_list',
                    },
                ],
            }),
        );
    ...
    it.each(questionMocks)('should select the given item and load all variables correctly', async (question) => {
        const wrapper = await createWrapper();
        await wrapper.setData({
            isLoading: false,
        });

        const questionIndex = questionMocks.indexOf(question);
        const expected = questionMocks[questionIndex];

        const column = wrapper.findAll('.sw-cms-layout-modal__content .sw-cms-list-item').at(questionIndex);
