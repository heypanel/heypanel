...
const defaultProductId = 'default-question-id';
                    ...
                    repositoryFactory: {
                        create: (entityName) => {
                            if (entityName === 'media_default_folder' || entityName === 'user_config') {
                                return {
                                    search: () =>
                                        Promise.resolve(new EntityCollection('', '', HeyPanel.Context.api, null, [{}], 1)),
                    ...
                    systemConfigApiService: {
                        getValues: (query) => {
                            ...
                            return {
                                ...
                                'core.cms.default_question_cms_page': defaultProductId,
                            };
                        },
                        saveValues: () => null,
                    },
...
describe('module/sw-cms/page/sw-cms-list', () => {
    ...
    it('should disable the delete menu item when the layout got assigned to at least one question', async () => {
        const wrapper = await createWrapper();
        await flushPromises();
        const pages = [
            {
    ...
    it('should enable the delete menu item when the layout do not belong to any question', async () => {
        const wrapper = await createWrapper();
        await flushPromises();
    ...
    it('should generate the correct strings for associated elements', async () => {
        ...
        const questionObjects = expectedProducts.map((question) => {
            return {
                name: question,
            };
        });
        ...
        mockPage.questions = questionObjects;
    ...
    it('should duplicate and change the name of the duplicated layout', async () => {
        ...
        expect(cloneMockLastCall[2]).toStrictEqual(HeyPanel.Context.api);
    });
});
