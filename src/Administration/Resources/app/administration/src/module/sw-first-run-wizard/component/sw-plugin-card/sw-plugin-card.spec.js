...
HeyPanel.Component.register('sw-extension-icon', SwExtensionIcon);
...
async function createWrapper(plugin, showDescription) {
    return mount(await wrapTestComponent('sw-plugin-card', { sync: true }), {
        ...
        global: {
            ...
            stubs: {
                'sw-extension-icon': await HeyPanel.Component.build('sw-extension-icon'),
...
describe('src/module/sw-first-run-wizard/component/sw-plugin-card', () => {
    ...
    it('can install a plugin', async () => {
        ...
        const extensionServiceSpy = jest.spyOn(wrapper.vm.heypanelExtensionService, 'updateExtensionData');
