...
const { Defaults, EntityDefinition } = HeyPanel;
...
export default {
    ...
    watch: {
        ...
        'country.forceStateInRegistration'(newVal) {
            if (!newVal) {
                HeyPanel.Store.get('error').removeApiError(
        ...
        'country.postalCodeRequired'(newVal) {
            if (!newVal) {
                HeyPanel.Store.get('error').removeApiError(`${this.address.getEntityName()}.${this.address.id}.zipcode`);
            }
