                    ...
                    repositoryFactory: {
                        create: (entityName) => {
                            ...
                            if (entityName === 'language') {
                                return {
                                    search: () =>
                                        Promise.resolve(new EntityCollection('', '', HeyPanel.Context.api, null, [], 0)),
                                    get: () => Promise.resolve(),
                                };
                            }
...
describe('modules/sw-users-permissions/page/sw-users-permissions-user-detail', () => {
    beforeAll(() => {
        HeyPanel.Service().register('timezoneService', () => {
            return new TimezoneService();
        });

        jest.spyOn(HeyPanel.ExtensionAPI, 'publishData').mockImplementation(() => {});
    });

    beforeEach(async () => {
        HeyPanel.Store.get('session').languageId = '123456789';
    ...
    afterEach(async () => {
        ...
        HeyPanel.Store.get('session').languageId = '';
    });
    ...
    it('should contain all fields', async () => {
        ...
        const fieldPhoneNumber = wrapper.findComponent('.sw-settings-user-detail__grid-phoneNumber');
        ...
        expect(fieldName.props('modelValue')).toBe('');
        expect(fieldPhoneNumber.props('modelValue')).toBe('');
        expect(fieldEmail.props('modelValue')).toBe('info@heypanel.net');
    ...
    it('should contain all fields with a given user', async () => {
        ...
        expect(fieldName.props('modelValue')).toBe('Max');
        expect(fieldPhoneNumber.props('modelValue')).toBe('Mustermann');
    ...
    it('should enable all fields when user has not editor rights', async () => {
        ...
        const fieldPhoneNumber = wrapper.find('.sw-settings-user-detail__grid-phoneNumber');
    ...
    it('should publish current user and editing user', async () => {
        expect(HeyPanel.ExtensionAPI.publishData).toHaveBeenCalledTimes(2);

        expect(HeyPanel.ExtensionAPI.publishData).toHaveBeenNthCalledWith(1, {
            id: 'sw-users-permissions-user-detail__currentUser',
            path: 'currentUser',
            scope: expect.anything(),
        });

        expect(HeyPanel.ExtensionAPI.publishData).toHaveBeenNthCalledWith(2, {
    ...
    it('should update the auth token if user password is changed', async () => {
        ...
        HeyPanel.Application.addServiceProvider('localeHelper', () => ({ setLocaleWithId: () => Promise.resolve() }));
