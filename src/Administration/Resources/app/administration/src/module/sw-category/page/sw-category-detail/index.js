...
const { Context, Mixin } = HeyPanel;
const { Criteria, ChangesetGenerator, EntityCollection } = HeyPanel.Data;
const { cloneDeep, merge } = HeyPanel.Utils.object;
const type = HeyPanel.Utils.types;
...
export default {
    ...
    data() {
        return {
            ...
            nextRoute: null,
            currentLanguageId: HeyPanel.Context.api.languageId,
    ...
    computed: {
        ...
        landingPage() {
            if (!HeyPanel.Store.get('swCategoryDetail')) {
                return {};
            }
        ...
        cmsPageState() {
            return HeyPanel.Store.get('cmsPage');
        },
    ...
    beforeCreate() {
        HeyPanel.Store.get('cmsPage').resetCmsPageState();
    },
    ...
    methods: {
        ...
        async setLandingPage() {
            ...
            try {
                if (this.landingPageId === null) {
                    ...
                    HeyPanel.Store.get('swCategoryDetail').landingPage = null;
                    this.cmsPageState.resetCmsPageState();

                    return;
                }

                await HeyPanel.Store.get('swCategoryDetail').loadActiveLandingPage({
                    repository: this.landingPageRepository,
                    apiContext: HeyPanel.Context.api,
        ...
        setCategory() {
            ...
            if (this.categoryId === null) {
                ...
                HeyPanel.Store.get('swCategoryDetail').category = null;
                this.cmsPageState.resetCmsPageState();
                this.isLoading = false;
                return;
            }

            HeyPanel.Store.get('swCategoryDetail')
                .loadActiveCategory({
        ...
        loadCustomFieldSet() {
            ...
            return this.customFieldSetRepository
                ...
                .then((customFieldSet) => {
                    HeyPanel.Store.get('swCategoryDetail').customFieldSets = customFieldSet;
                })
                .finally(() => {
                    this.isCustomFieldLoading = true;
                });
        ...
        onLeaveModalConfirm(destination) {
            ...
            HeyPanel.Store.get('error').removeApiError('category');
        ...
        async onSave() {
            ...
            return this.categoryRepository
                .save(this.category, { ...HeyPanel.Context.api })
                .then(() => {
        ...
        onSaveLandingPage() {
            ...
            return this.landingPageRepository
                .save(this.landingPage, HeyPanel.Context.api)
                .then(() => {
                    this.isSaveSuccessful = true;
        ...
        addLandingPageChannelError() {
            const heypanelError = new HeyPanel.Classes.HeyPanelError({
        ...
        updateSeoUrls() {
            if (!HeyPanel.Store.list().includes('swSeoUrl')) {
                return Promise.resolve();
            }

            const seoUrls = HeyPanel.Store.get('swSeoUrl').newOrModifiedUrls;
        ...
        onLandingPageDelete() {
            HeyPanel.Store.get('swCategoryDetail').landingPagesToDelete = null;
        },

        onCategoryDelete() {
            HeyPanel.Store.get('swCategoryDetail').categoriesToDelete = null;
        },
    },
};
