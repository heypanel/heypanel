...
import type { HeyPanelDiscountCampaignService } from 'src/app/service/discount-campaign.service';
import type {
 ...
 * @private
 */
export default class HeyPanelExtensionService {
    ...
    constructor(
        ...
        private readonly discountCampaignService: HeyPanelDiscountCampaignService,
    ...
    public async updateExtensionData(refreshExtensions: boolean = true): Promise<void> {
        HeyPanel.Store.get('heypanelExtensions').loadMyExtensions();

        try {
            if (!HeyPanel.Store.get('context').app.config?.settings?.disableExtensionManagement && refreshExtensions) {
            ...
            HeyPanel.Store.get('heypanelExtensions').setMyExtensions(myExtensions);
        } finally {
            HeyPanel.Store.get('heypanelExtensions').setLoading(false);
        }
    }
    ...
    public async getOpenLink(extension: Extension): Promise<null | LabeledLocation | RouteLocationNamedRaw> {
        ...
        const entryRoutes = HeyPanel.Store.get('extensionEntryRoutes').routes;
    ...
    private async getLinkToTheme(extension: Extension) {
        ...
        const themeRepository = HeyPanel.Service('repositoryFactory').create('theme');
