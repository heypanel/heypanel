...
export default {
    ...
    computed: {
        ...
        extensionMainModule() {
            return HeyPanel.Store.get('extensionMainModules').mainModules.find(
        ...
        extensionManagementDisabled() {
            return HeyPanel.Store.get('context').app.config.settings.disableExtensionManagement;
        },
    ...
    methods: {
        async createdComponent() {
            this.openLink = await this.heypanelExtensionService.getOpenLink(this.extension);
        },
        ...
        async closeModalAndUninstallExtension(removeData) {
            ...
            try {
                await this.heypanelExtensionService.uninstallExtension(this.extension.name, this.extension.type, removeData);
        ...
        async updateExtension(allowNewPermissions = false) {
            ...
            try {
                ...
                if (this.extension.installedAt) {
                    await this.heypanelExtensionService.updateExtension(
                        this.extension.name,
                        this.extension.type,
                        allowNewPermissions,
                    );
        ...
        async removeExtension(removeData) {
            try {
                ...
                await this.heypanelExtensionService.removeExtension(this.extension.name, this.extension.type, removeData);
