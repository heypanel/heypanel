...
describe('src/module/sw-extension/page/sw-extension-config.spec', () => {
                    ...
                    systemConfigApiService: {
                        getValues: () => {
                            return Promise.resolve({
                                ...
                                'core.store.heypanelId': 'max@muster.com',
                            });
                        },
                    },
                },
    ...
    it('should reload extensions on createdComponent', async () => {
        ...
        expect(wrapper.vm.heypanelExtensionService.updateExtensionData).toHaveBeenCalledTimes(1);
    });

    it('should not reload extensions on createdComponent if extensions are loaded', async () => {
        HeyPanel.Store.get('heypanelExtensions').setMyExtensions([{ name: 'test-extension' }]);
        const wrapper = await createWrapper();

        expect(wrapper.vm.heypanelExtensionService.updateExtensionData).toHaveBeenCalledTimes(0);
    });
    ...
    it('shows header for extension details', async () => {
        ...
        const meta = wrapper.get('.sw-meteor-page__smart-bar-meta');
        expect(meta.text()).toBe('sw-extension-store.component.sw-extension-config.labelBy heypanel AG');
    });
    ...
    it('shows header for extension details with producer website', async () => {
        ...
        wrapper.vm.extension = {
            producerName: 'heypanel AG',
            producerWebsite: 'https://www.heypanel.com/',
        };
        ...
        expect(metaLink.attributes().href).toBe('https://www.heypanel.com/');
        expect(metaLink.text()).toBe('heypanel AG');
    });
