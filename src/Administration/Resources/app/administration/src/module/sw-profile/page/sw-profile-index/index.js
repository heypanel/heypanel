...
const { Component, Mixin, Store } = HeyPanel;
...
export default {
    ...
    computed: {
        searchPreferences: () => HeyPanel.Store.get('swProfile').searchPreferences,
    ...
    methods: {
        ...
        loadLanguages() {
            ...
            return this.languageRepository.search(languageCriteria).then((result) => {
                ...
                result.forEach((lang) => {
                    ...
                    if (lang.locale.code === HeyPanel.Context.app.fallbackLocale) {
            ...
            if (!this.acl.can('user:editor')) {
                ...
                this.userService
                    ...
                    .then(async () => {
                        ...
                        HeyPanel.Service('localeHelper').setLocaleWithId(this.user.localeId);
                    })
                    .catch((error) => {
                        HeyPanel.Store.get('error').addApiError({
                            expression: `user.${this.user?.id}.password`,
                            error: new HeyPanel.Classes.HeyPanelError(error.response.data.errors[0]),
                        });
                        this.createNotificationError({
        ...
        updateCurrentUser() {
            return this.userService.getUser().then((response) => {
                ...
                return HeyPanel.Store.get('session').setCurrentUser(data);
            });
        },
