...
describe('src/module/sw-settings-number-range/page/sw-settings-number-range-create', () => {
    ...
    it('should be able show question warning alert when number range is global', async () => {
        const wrapper = await createWrapper();
        ...
        const questionAlert = wrapper.find('.sw-number_range-quickinfo__question-alert');
        expect(questionAlert.exists()).toBe(false);
        expect(wrapper.vm.isShowProductWarning).toBe(false);
        expect(loadChannelsSpy).toHaveBeenCalled();

        await selectType.trigger('change', { technicalName: 'question' });
        await flushPromises();
