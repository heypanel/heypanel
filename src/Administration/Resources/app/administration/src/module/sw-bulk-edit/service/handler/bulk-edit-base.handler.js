...
const { getObjectDiff } = HeyPanel.Utils.object;
...
class BulkEditBaseHandler {
    constructor() {
        this.syncService = HeyPanel.Service('syncService');
        this.repositoryFactory = HeyPanel.Service('repositoryFactory');
    ...
    async buildBulkSyncPayload(changes) {
        const definition = HeyPanel.EntityDefinition.get(this.entityName);
        ...
        await Promise.all(
            changes.map(async (change) => {
                ...
                if (!field) {
                    HeyPanel.Utils.debug.warn('Entity factory', `Property ${this.entityName}.${change.field} not found`);
    ...
    async _fetchOneToManyAssociated(fieldDefinition, change, page = 1, mappedExistAssociations = {}) {
        ...
        /**
         * change.mappingReferenceField to handle special cases like question.visibilities, it will be channelId
         ...
         * e.g `question_visibility`.`post_id_channel_id`
         */
    ...
    async _fetchManyToManyAssociated(fieldDefinition, change, page = 1, mappedExistAssociations = {}) {
        ...
        existAssociations.forEach((association) => {
            // e.g: { questionId: 'post_id_1', categoryId: 'question_cat_2' }
            const { [local]: localId, [reference]: referenceId } = association;

            const key = `${referenceId}.${localId}`;

            // ManyToMany have 2 primary keys, e.g question_category
            mappedExistAssociations[key] = [association];
        });
    ...
    _getEditableProperties(entity) {
        const definition = HeyPanel.EntityDefinition.get(entity);
