...
describe('src/module/sw-bulk-edit/page/sw-bulk-edit-member', () => {
    let wrapper;
    let routes;
    ...
    async function createWrapper(isResponseError = false) {
        ...
        return mount(await wrapTestComponent('sw-bulk-edit-member', { sync: true }), {
            global: {
                plugins: [
                    router,
                ],
    ...
    beforeAll(async () => {
        routes = [
            ...
            },
            {
                ...
                meta: {
                    $module: {
                        title: 'sw-bulk-edit-member.general.mainMenuTitle',
                    },
                },
                redirect: {
                ...
                children: [
                    ...
                    {
                        name: 'sw.bulk.edit.member.save.process',
                        path: '/process',
                    ...
                    {
                        name: 'sw.bulk.edit.member.save.success',
                        path: '/success',
                    ...
                    {
                        name: 'sw.bulk.edit.member.save.error',
                        path: '/error',
    ...
    beforeEach(async () => {
        ...
        HeyPanel.Store.get('swBulkEdit').selectedIds = [
            HeyPanel.Utils.createId(),
        ];
    });
    ...
    it('should be show empty state', async () => {
        ...
        expect(emptyState.find('.sw-empty-state__title').text()).toBe('sw-bulk-edit.member.messageEmptyTitle');
    });
    ...
    it('should open confirm modal', async () => {
        ...
        await wrapper.find('.sw-bulk-edit-member__save-action').trigger('click');
    ...
    it('should create new instance for bulk handler', async () => {
        ...
        const tagsCard = wrapper.find('.sw-bulk-edit-member-base__tags');
        expect(tagsCard.exists()).toBeTruthy();

        await tagsCard.find('.sw-bulk-edit-change-field__change input').trigger('click');
        await flushPromises();

        HeyPanel.Store.get('swBulkEdit').selectedIds = new Array(100).fill(1);
        ...
        // Check if getHandler was called with 'member'
        expect(wrapper.vm.bulkEditApiFactory.getHandler).toHaveBeenCalledWith('member');
    ...
    it('should show tags and custom fields card', async () => {
        ...
        const customFieldsCard = wrapper.find('.sw-bulk-edit-member-base__custom_fields');
    ...
    it('should show account card', async () => {
        ...
        const accountCard = wrapper.find('.sw-bulk-edit-member-base__account');
