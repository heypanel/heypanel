                            ...
                            if (entity === 'question') {
                                return Promise.resolve(
                                    [
                                        ...
                                        return criteria.ids.includes(question.id);
                                    }),
                                );
                            }
                            if (entity === 'media') {
...
function createConfig() {
    const firstCardElements = [
        ...
        },
        {
            ...
            type: 'url',
            config: {
                defaultValue: 'https://www.heypanel.com',
                label: {
                    'en-GB': 'url field',
                },
            },
            _test: {
                defaultValueDom: 'www.heypanel.com',
                domValueCheck: (field, domValue) => {
                    expect(field.find('input').element.value).toBe(domValue);
                },
                afterValue: 'https://www.heypanel.de',
                afterValueDom: 'www.heypanel.de',
                childValue: 'https://www.child.heypanel.com',
                childValueDom: 'www.child.heypanel.com',
                changeValueFunction: async (field, afterValue) => {
                    // change input value
...
describe('src/module/sw-settings/component/sw-system-config/sw-system-config', () => {
    ...
    it('should show a select field for the channels', async () => {
        wrapper = await createWrapper();
        await flushPromises();
    ...
    it('should change the channel', async () => {
        wrapper = await createWrapper();
        await flushPromises();
    ...
    it('should return HeyPanelError when has error', async () => {
        HeyPanel.Store.get('error').addApiError({
            expression: 'SYSTEM_CONFIG.null.dummyKey',
            error: new HeyPanelError({ code: 'dummyCode' }),
        });
        ...
        expect(error).toBeInstanceOf(HeyPanelError);
    });
    ...
    createConfig()[0].elements.forEach(({ name, type, config, _test }) => {
        ...
        it(`should render field with type "${type || name}" with the inherit value and should be able to remove the inheritance`, async () => {
            ...
            // check if headless channel is activated
            selectionText = channelSwitch.find('.sw-entity-single-select__selection-text');
            expect(selectionText.text()).toBe('Headless');
