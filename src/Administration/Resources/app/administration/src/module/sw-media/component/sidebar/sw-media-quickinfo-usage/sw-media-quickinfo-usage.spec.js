...
describe('module/sw-media/components/sw-media-quickinfo-usage', () => {
    ...
    it('should be correct to show media in used information when user select a media', async () => {
        register('sw-question', moduleMock);
        const postMediaMock = {
        ...
        await wrapper.setProps({
            item: itemDeleteMock({ postMedia: [postMediaMock] }),
        });
        expect(wrapper.vm.getUsages.some((usage) => usage.name === postMediaMock.question.translated.name)).toBeTruthy();
    });
    ...
    it('should be correct show all of media in used information', async () => {
        ...
        register('sw-question', moduleMock);
        const postMediaMock = {
            question: { translated: { name: 'question test' } },
        };
        ...
        await wrapper.setProps({
            item: itemDeleteMock({
                ...
                postMedia: [postMediaMock],
                categories: [categoryMock],
                questionManufacturers: [manufacturerMock],
        ...
        expect(usages.some((usage) => usage.name === postMediaMock.question.translated.name)).toBeTruthy();
