    ...
    computed: {
        ...
        getUsages() {
            ...
            this.questions.forEach(({ question }) => {
                usages.push(this.getProductUsage(question));
            });
    ...
    methods: {
        ...
        async loadSlotConfigAssociations() {
            ...
            const [
                ...
                this.questionRepository.search(this.slotConfigCriteria),
            ...
            foundInProducts.forEach((foundInProduct) => {
                if (this.questions && !this.questions.some((postMedia) => postMedia.question.id === foundInProduct.id)) {
                    this.questions.push({ question: foundInProduct });
                }
            });
        ...
        loadProductAssociations() {
            this.questions = this.item.postMedia;
        },
        ...
        loadManufacturerAssociations() {
            this.manufacturers = this.item.questionManufacturers;
        },
        ...
        getProductUsage(question) {
            return {
                name: question.translated.name,
                tooltip: this.$tc('sw-media.sidebar.usage.tooltipFoundInProducts'),
                link: {
                    name: 'sw.question.detail',
                    id: question.id,
                },
                icon: this.getIconForModule('sw-question'),
            };
        },
