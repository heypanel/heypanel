...
function getCollection(repository, entities = [], aggregations = defaultAggregations, total = 0) {
    return new EntityCollection(
        ...
        repository,
        Context.api,
        { isHeyPanelContext: true, page: 1, limit: 25 },
        entities,
        total,
        aggregations,
    );
...
describe('src/module/sw-settings-rule/page/sw-settings-rule-detail', () => {
    ...
    it.each([
        { name: 'question association', entity: 'question' },
        { name: 'no question association', entity: 'order' },
    ...
    it.each([
        ...
        const apiLanguageId = HeyPanel.Store.get('context').api.languageId;
        expect(HeyPanel.Store.get('context').api.languageId).not.toBe('uuid1');
        ...
        expect(HeyPanel.Store.get('context').api.languageId).toBe(abort ? apiLanguageId : 'uuid1');

        // cleanup
        HeyPanel.Store.get('context').api.languageId = apiLanguageId;
    });
