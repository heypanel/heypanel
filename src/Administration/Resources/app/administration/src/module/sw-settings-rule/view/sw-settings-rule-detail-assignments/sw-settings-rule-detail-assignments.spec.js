...
const testConfig = {
    question: {
        id: 'question',
        associationName: 'questionPrices',
        notAssignedDataTotal: 1,
        allowAdd: true,
        entityName: 'question',
        label: 'sw-settings-rule.detail.associations.questions',
        criteria: () => {
        ...
        detailRoute: 'sw.question.detail.prices',
        addContext: {
            type: 'test',
        },
        gridColumns: [
            {
                ...
                rawData: true,
                sortable: true,
                routerLink: 'sw.question.detail.prices',
                allowEdit: false,
            },
        ],
    },
...
describe('src/module/sw-settings-rule/view/sw-settings-rule-detail-assignments', () => {
    ...
    it.each([
        ...
        ruleAssignmentServiceMock.getConfiguration.mockImplementationOnce(() => {
            return {
                ...
                question: {
                    ...testConfig.question,
                    api: defaultApi ? null : testConfig.question.api,
                },
            };
        });
        ...
        expect(repository.search).toHaveBeenNthCalledWith(
            1,
            expect.any(Criteria),
            defaultApi ? Context.api : testConfig.question.api(),
        );
        expect(wrapper.vm.associationEntities).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
    ...
    it('should throw error if loading association data fails', async () => {
        const wrapper = await createWrapper(defaultProps, ['question'], {
            search: jest.fn(() => {
    ...
    it('should render an entity-listing for each entity when all entities have results', async () => {
        ...
        expect(wrapper.find('.sw-settings-rule-detail-assignments__card-question .router-link').exists()).toBe(true);

        // Empty states should not be present
        expect(wrapper.find('.sw-settings-rule-detail-assignments__entity-empty-state-question').exists()).toBe(false);
    ...
    it('should render an entity-listing also if no assignment is found', async () => {
        ...
        expect(wrapper.find('.sw-settings-rule-detail-assignments__card-question .router-link').exists()).toBe(false);

        // Expect empty states to be present
        expect(wrapper.find('.sw-settings-rule-detail-assignments__entity-empty-state-question img').exists()).toBe(true);
    ...
    it('should render names of question variants', async () => {
        const wrapper = await await createWrapper();
        await flushPromises();

        // expect entity listing for questions to be present
        expect(wrapper.find('.sw-settings-rule-detail-assignments__card-question .router-link').exists()).toBeTruthy();

        const questionAssignments = wrapper.findAll(
            '.sw-settings-rule-detail-assignments__entity-listing-question .sw-data-grid__cell--name',
        );

        // expect the right amount of items
        expect(questionAssignments).toHaveLength(4);
        ...
        // expect the correct names of the questions
        questionAssignments.forEach((assignment, index) => {
    ...
    it('should have the right link inside the template', async () => {
        ...
        const questionListing = wrapper.find(
            '.sw-settings-rule-detail-assignments__entity-listing-question .sw-data-grid__cell--name .router-link',
        );

        // expect promotion entity listing to exist
        expect(questionListing.exists()).toBe(true);
        const detailRouteAttribute = questionListing.attributes('detail-route');

        // expect detail-route attribute to be correct
        expect(detailRouteAttribute).toBe(testConfig.question.detailRoute);
    });
    ...
    it.each([
        ...
        const wrapper = await createWrapper(defaultProps, total === 0 ? [] : ['question']);
        await flushPromises();
    ...
    it.each([
        ...
        ruleAssignmentServiceMock.getConfiguration.mockImplementationOnce(() => {
            return {
                ...
                flow: {
                    ...
                    gridColumns: testConfig.question.gridColumns,
                },
            };
        });
        ...
        assignedTotalCriteria.addFilter(Criteria.not('AND', testConfig.question.criteria().filters));
        ...
        expect(apis).toContainEqual(defaultApi ? Context.api : testConfig.question.api());
    });
    ...
    it.each([
        ...
        const itemMock = {
            ...
            getEntityName: () => 'question',
        };

        const repositoryOverwriteMock = {
            search: jest.fn(() => Promise.resolve(createEntityCollectionMock('question', [itemMock]))),
            save: jest.fn(() => Promise.resolve()),
        };

        const wrapper = await createWrapper(defaultProps, ['question'], repositoryOverwriteMock);
        await flushPromises();
    ...
    it.each([
        ...
        const criteria = testConfig.question.criteria();
        criteria.setPage(1);
        criteria.setTerm(term);

        expect(repository.search).toHaveBeenCalledWith(criteria, defaultApi ? Context.api : testConfig.question.api());
    ...
    it('should set router link', async () => {
        ...
        expect(wrapper.find('.sw-data-grid__row--0 .router-link').attributes('detail-route')).toBe(
            testConfig.question.detailRoute,
        );
    });
});
