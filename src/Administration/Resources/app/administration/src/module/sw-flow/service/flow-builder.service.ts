...
const { Utils, EntityDefinition } = HeyPanel;
const { capitalizeString, camelCase, snakeCase } = HeyPanel.Utils.string;
...
type ActionData = {
    ...
    memberGroups: EntityCollection<'member_group'>;
...
export default class FlowBuilderService {
    ...
    private $entityAction = {
        ...
        [this.$actionNames.ADD_CUSTOMER_TAG]: 'member',
        [this.$actionNames.SET_ORDER_CUSTOM_FIELD]: 'order',
        [this.$actionNames.REMOVE_CUSTOMER_TAG]: 'member',
        [this.$actionNames.SET_CUSTOMER_CUSTOM_FIELD]: 'member',
        [this.$actionNames.ADD_ORDER_AFFILIATE_AND_CAMPAIGN_CODE]: 'order',
        [this.$actionNames.SET_CUSTOMER_GROUP_CUSTOM_FIELD]: 'member_group',
        [this.$actionNames.ADD_CUSTOMER_AFFILIATE_AND_CAMPAIGN_CODE]: 'member',
    };
    ...
    public getCustomerStatusDescription(context: ActionContext) {
        ...
        return config.active
            ? translator.$tc('sw-flow.modals.memberStatus.active')
            : translator.$tc('sw-flow.modals.memberStatus.inactive');
    }
    ...
    public getCustomerGroupDescription(context: ActionContext) {
        ...
        const memberGroup = data.memberGroups.find((item) => item.id === config.memberGroupId);
        return memberGroup?.translated?.name;
    }
