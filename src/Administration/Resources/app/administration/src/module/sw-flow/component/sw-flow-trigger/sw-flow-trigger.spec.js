...
const mockBusinessEvents = [
    {
        name: 'checkout.member.before.login',
        mailAware: true,
        aware: ['HeyPanel\\Core\\Framework\\Event\\ChannelAware'],
    },
    {
        name: 'checkout.member.changed-payment-method',
        mailAware: false,
        aware: ['HeyPanel\\Core\\Framework\\Event\\ChannelAware'],
    },
    {
        name: 'checkout.member.deleted',
        mailAware: true,
        aware: ['HeyPanel\\Core\\Framework\\Event\\ChannelAware'],
    },
    {
...
const mockTranslations = {
    ...
    'sw-flow.triggers.member': 'Customer',
...
describe('src/module/sw-flow/component/sw-flow-trigger', () => {
    beforeAll(() => {
        HeyPanel.Service().register('ruleConditionDataProviderService', () => {
            return {
                getRestrictedRules: () => Promise.resolve([]),
            };
        });

        HeyPanel.Service().register('businessEventService', () => {
            return {
    ...
    it('should display event tree with event get from flow state', async () => {
        HeyPanel.Store.get('swFlow').triggerEvents = mockBusinessEvents;
        ...
        eventTree = wrapper.find('.sw-tree');
        expect(eventTree.exists()).toBeTruthy();
        HeyPanel.Store.get('swFlow').triggerEvents = [];
    });
    ...
    it('should get event tree data correctly', async () => {
        ...
        expect(wrapper.vm.eventTree).toEqual([
            ...
            {
                childCount: 1,
                id: 'checkout.member.before',
                name: 'Before',
                parentId: 'checkout.member',
                disabled: false,
                disabledToolTipText: null,
            },
            {
                childCount: 0,
                id: 'checkout.member.before.login',
                name: 'Login',
                parentId: 'checkout.member.before',
                disabled: false,
                disabledToolTipText: null,
            },
            {
                childCount: 0,
                id: 'checkout.member.changed-payment-method',
    ...
    it('should emit an event when clicking tree item', async () => {
        ...
        expect(emittedEvent[0]).toEqual(['checkout.member.deleted']);
    });
    ...
    it('should emit an event when clicking on search item', async () => {
        ...
        expect(emittedEvent[0]).toEqual([
            'checkout.member.changed-payment-method',
        ]);
    });
    ...
    it('should able to emit an event when pressing Enter on the item which has no children', async () => {
        ...
        // Press enter to select member item
        window.document.dispatchEvent(
            new KeyboardEvent('keydown', {
                key: 'Enter',
            }),
    ...
    it('should emit an event when pressing Enter on search item', async () => {
        ...
        const emittedEvent = wrapper.emitted()['option-select'];
        expect(emittedEvent).toBeTruthy();
        expect(emittedEvent[0]).toEqual(['checkout.member.before.login']);
    });
    ...
    it('should show confirmation modal when clicking tree item', async () => {
        ...
        const isSequenceEmpty = HeyPanel.Store.get('swFlow').isSequenceEmpty;
    ...
    it('should get event tree data correctly with custom event', async () => {
        ...
        mockBusinessEvents.push({
            ...
            mailAware: true,
            aware: ['HeyPanel\\Core\\Framework\\Event\\CustomEventAware'],
        });
