...
const { Component, Mixin, Context, Store, Utils, Service } = HeyPanel;
...
export default {
    ...
    methods: {
        ...
        getAppFlowAction() {
            return this.appFlowActionRepository.search(this.appFlowActionCriteria, HeyPanel.Context.api).then((response) => {
        ...
        getDataForActionDescription() {
            ...
            if (hasDocumentAction) {
                ...
                promises.push(
                    this.documentTypeRepository.search(this.documentTypeCriteria).then((data) => {
                        HeyPanel.Store.get('swFlow').documentTypes = data;
                    }),
                );
            }
            ...
            if (hasMailSendAction) {
                ...
                promises.push(
                    this.mailTemplateRepository.search(this.mailTemplateIdsCriteria).then((data) => {
                        HeyPanel.Store.get('swFlow').mailTemplates = data;
                    }),
                );
            }
            ...
            if (hasChangeCustomerGroup) {
                // get support information for change member group action.
                promises.push(
            ...
            if (hasSetCustomFieldAction) {
                promises.push(
                    this.customFieldSetRepository.search(this.customFieldSetCriteria).then((data) => {
                        HeyPanel.Store.get('swFlow').customFieldSets = data;
                    }),
                );

                promises.push(
                    this.customFieldRepository.search(this.customFieldCriteria).then((data) => {
                        HeyPanel.Store.get('swFlow').customFields = data;
                    }),
                );
            }
