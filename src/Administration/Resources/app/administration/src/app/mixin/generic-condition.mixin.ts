    ...
    defineComponent({
        ...
        computed: {
            config(): Config {
                ...
                const config = HeyPanel.Store.get('ruleConditionsConfig').getConfigForType(this.condition.type as string) as
                    | Config
                    | undefined;
            ...
            currentError() {
                ...
                Object.values(this.config.fields).forEach((config) => {
                    ...
                    const errorProperty = HeyPanel.Store.get('error').getApiError(
                        // @ts-expect-error
        ...
        methods: {
            getBind(field: Field) {
                const fieldClone = HeyPanel.Utils.object.cloneDeep(field);
