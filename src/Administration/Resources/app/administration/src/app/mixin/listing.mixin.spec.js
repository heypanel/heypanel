...
async function createWrapper({
    ...
    return mount(
        ...
        `,
            mixins: [
                HeyPanel.Mixin.getByName('listing'),
            ],
            data() {
                return {
                    ...defaultData,
                    ...
                    return [
                        {
                            name: 'term',
                            property: 'name',
                            label: 'sw-question.list.columnName',
                            active: false,
                        },
                    ];
                },
...
describe('src/app/mixin/listing.mixin.ts', () => {
    ...
    beforeEach(async () => {
        ...
        if (originalWarn) {
            HeyPanel.Utils.debug.warn = originalWarn;
        } else {
            originalWarn = HeyPanel.Utils.debug.warn;
        }
    });
    ...
    it('should throw a console warning when no getList method ist defined', async () => {
        ...
        HeyPanel.Utils.debug.warn = jest.fn(() => {});
        ...
        expect(HeyPanel.Utils.debug.warn).toHaveBeenCalledWith(
            'Listing Mixin',
    ...
    it('should have the correct routeName computed value', async () => {
        expect(wrapper.vm.routeName).toBe('sw.question.index');

        await router.push({
            name: 'sw.question.detail',
        });

        expect(wrapper.vm.routeName).toBe('sw.question.detail');
    });
