...
async function createWrapper(attrs = {}) {
    return mount(
        ...
        `,
            mixins: [
                HeyPanel.Mixin.getByName('remove-api-error'),
            ],
            data() {
                return {
                    name: 'sw-mock-field',
...
describe('src/app/mixin/remove-api-error.mixin.ts', () => {
    ...
    beforeEach(async () => {
        if (originalDispatch) {
            Object.defineProperty(HeyPanel.State, 'dispatch', {
                value: originalDispatch,
            });
        } else {
            originalDispatch = HeyPanel.State.dispatch;
        }
    ...
    it('should call  removeApiError on value change', async () => {
        ...
        const errorStore = HeyPanel.Store.get('error');
        ...
        expect(HeyPanel.Store.get('error').removeApiError).toHaveBeenCalledWith('self.link');
    });
});
