    ...
    defineComponent({
        methods: {
            placeholder<EntityName extends keyof EntitySchema.Entities>(
                ...
                if (HeyPanel.Utils.types.isString(entity[field]) && entity[field].length > 0) {
                    return entity[field];
                }

                // Return the field from parent translation if set
                const parentLanguageId = HeyPanel.Context.api.language ? HeyPanel.Context.api.language.parentId : null;
