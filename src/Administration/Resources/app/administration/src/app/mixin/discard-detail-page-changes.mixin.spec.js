...
async function createWrapper(...entityNames) {
    ...
    return mount(
        ...
        `,
            mixins: [
                HeyPanel.Mixin.getByName('discard-detail-page-changes')(...entityNames),
            ],
            data() {
                return {
                    question: {
...
describe('src/app/mixin/discard-detail-page-changes.mixin.ts', () => {
    ...
    beforeEach(async () => {
        wrapper = await createWrapper('question');
    ...
    it('should call the entity discardChanges function on route params id change', async () => {
        expect(wrapper.vm.question.discardChanges).not.toHaveBeenCalled();
        ...
        expect(wrapper.vm.question.discardChanges).toHaveBeenCalledWith();
    });
    ...
    it('should log a warning when not entity with the name has a discard method was found', async () => {
        ...
        jest.spyOn(HeyPanel.Utils.debug, 'warn').mockImplementationOnce(() => {});
