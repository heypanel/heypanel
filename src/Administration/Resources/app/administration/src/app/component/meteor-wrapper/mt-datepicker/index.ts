import { MtDatepicker as MtDatepickerOriginal } from '@heypanel-ag/meteor-component-library';
...
export default HeyPanel.Component.wrapComponentConfig({
    ...
    computed: {
        userLocale(): string {
            return HeyPanel.Store.get('session').currentLocale || 'en-US';
        },
        ...
        is24HourFormat(): boolean {
            ...
            const locale = HeyPanel.Store.get('session').currentLocale!;
    ...
    methods: {
        customFormat(date: Date): string {
            const currentLocale = HeyPanel.Store.get('session').currentLocale || 'en-US';
