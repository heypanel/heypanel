...
export default HeyPanel.Component.wrapComponentConfig({
    ...
    created() {
        this.heightHandler = HeyPanel.ExtensionAPI.handle('locationUpdateHeight', ({ height, locationId }) => {
            if (locationId === this.locationId) {
                this.locationHeight = Number(height) ?? null;
            }
        });

        this.urlHandler = HeyPanel.ExtensionAPI.handle(
            'locationUpdateUrl',
    ...
    computed: {
        ...
        componentName(): string | undefined {
            return HeyPanel.Store.get('sdkLocation').locations[this.locationId];
        },

        extension(): Extension | undefined {
            const extensions = HeyPanel.Store.get('extensions').extensionsState;
