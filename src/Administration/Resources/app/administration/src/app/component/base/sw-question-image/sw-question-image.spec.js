...
async function createWrapper() {
    return mount(await wrapTestComponent('sw-question-image', { sync: true }), {
        global: {
            stubs: {
                'sw-label': {
                    template: `
...
describe('app/component/base/sw-question-image', () => {
    it('should display button to set as cover image if media is not a 3D object and is not already a cover image', async () => {
        ...
        const setAsCoverButton = wrapper.find('.sw-question-image__context-button .sw-question-image__button-cover');
    ...
    it('should not show spatial label if media item is not a 3D object', async () => {
        ...
        const setAsCoverButton = wrapper.find('.sw-question-image__spatial-label .sw-question-image__button-cover');
        expect(setAsCoverButton.exists()).toBe(false);
    });
    ...
    it('should show spatial label if media item is a 3D object', async () => {
        ...
        const setAsCoverButton = wrapper.find('.sw-question-image__spatial-label .sw-label__spatial');
        expect(setAsCoverButton.exists()).toBe(true);
    });
    ...
    it('should should AR ready label if media item is a 3D object and has AR enabled', async () => {
        ...
        const setAsCoverButton = wrapper.find('.sw-question-image__spatial-label .sw-label__ar-ready');
        expect(setAsCoverButton.exists()).toBe(true);
    });
});
