...
async function createWrapper(options = {}) {
    const router = createRouter({
        routes: [
            ...HeyPanel.Module.getModuleRoutes(),
            {
...
describe('src/app/component/structure/sw-admin-menu', () => {
    ...
    beforeAll(() => {
        ...
        HeyPanel.Context.app.fallbackLocale = 'en-GB';

        HeyPanel.Module.getModuleRegistry().clear();
        adminModules.forEach((adminModule) => {
            HeyPanel.Module.register(adminModule.name, adminModule);
        });
    });
    ...
    beforeEach(async () => {
        ...
        jest.spyOn(HeyPanel.Utils.debug, 'error').mockImplementation(() => true);

        HeyPanel.Store.get('session').setCurrentUser(null);
    ...
    it('should not render 4.level or higher menu item and throw error', async () => {
        ...
        expect(HeyPanel.Utils.debug.error.mock.calls[0][0]).toBeInstanceOf(Error);
        expect(HeyPanel.Utils.debug.error.mock.calls[0][0].toString()).toBe(
        ...
        expect(HeyPanel.Utils.debug.error.mock.calls[1][0]).toBeInstanceOf(Error);
        expect(HeyPanel.Utils.debug.error.mock.calls[1][0].toString()).toBe(
    ...
    describe('app menu entries', () => {
        it('renders apps under there parent navigation entry', async () => {
            HeyPanel.Store.get('heypanelApps').apps = testApps;
            await flushPromises();
