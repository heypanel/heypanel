...
const linkDataProvider = [
    ...
    {
        buttonConfig: {
            value: 'mailto:test@heypanel.com',
            type: 'email',
        },
        value: 'test@heypanel.com',
        type: 'email',
        prefix: 'mailto:',
...
describe('components/form/sw-text-editor/sw-text-editor-link-menu', () => {
    ...
    it('parses question detail links and reacts to changes correctly', async () => {
        const wrapper = await createWrapper({
            value: `${seoDomainPrefix}/detail/aaaaaaa524604ccbad6042edce3ac799#`,
            type: 'detail',
        });
        ...
        const questionSingleSelectInput = wrapper.find('.sw-text-editor-link-menu__entity-single-select input');
        await questionSingleSelectInput.trigger('click');

        await flushPromises();

        expect(wrapper.text()).toContain('sw-text-editor-toolbar.link.linkTo');
        expect(questionSingleSelectInput.element.placeholder).toBe('sw-text-editor-toolbar.link.placeholderProduct');

        const questionSingleSelect = wrapper.findComponent('.sw-entity-single-select').vm;
        expect(questionSingleSelect.entity).toBe('question');
        expect(questionSingleSelect.value).toBe('aaaaaaa524604ccbad6042edce3ac799');

        expect(questionSingleSelect.criteria).toStrictEqual(
        ...
        const associations = questionSingleSelect.criteria.associations;
        ...
        expect(questionSingleSelect.criteria.filters).toStrictEqual(
            expect.objectContaining([
                {
                    operator: 'OR',
                    queries: [
                        {
                            field: 'question.childCount',
                            type: 'equals',
                            value: 0,
                        },
                        {
        ...
        const results = questionSingleSelect.resultCollection;
        expect(results).toHaveLength(2);
        expect(results[0]).toEqual(questionData[0].attributes);
        expect(results[1]).toEqual(questionData[1].attributes);

        // Valid value set
        await questionSingleSelect.setValue(questionData[1]);
        await wrapper.find('.sw-text-editor-toolbar-button__link-menu-buttons-button-insert').trigger('click');
        await flushPromises();
        ...
        // No value set
        await questionSingleSelect.setValue({ id: null });
        await flushPromises();
