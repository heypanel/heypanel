...
describe('components/rule/condition-type/sw-condition-generic', () => {
    beforeEach(() => {
        HeyPanel.Store.get('ruleConditionsConfig').config = ruleConditionsConfig;
    });

    it('should render fields and set condition values on change', async () => {
        const wrapper = await createWrapper({
            type: 'memberCustomerGroup',
        });
        await flushPromises();

        expect(wrapper.vm.condition.value.operator).toBeUndefined();
        expect(wrapper.vm.condition.value.memberGroupIds).toBeUndefined();
        expect(wrapper.vm.values.memberGroupIds).toEqual([]);
        ...
        expect(wrapper.vm.condition.value.memberGroupIds).toEqual(
