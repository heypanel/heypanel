...
export default {
    ...
    computed: {
        languageId() {
            return HeyPanel.Store.get('context').api.languageId;
        },

        systemLanguageId() {
            return HeyPanel.Store.get('context').api.systemLanguageId;
        },

        language() {
            return HeyPanel.Store.get('context').api.language;
        },
    ...
    methods: {
        async refreshParentLanguage() {
            ...
            if (this.language.parentId !== null && this.language.parentId.length > 0) {
                this.parentLanguage = await this.languageRepository.get(this.language.parentId, HeyPanel.Context.api);
                return;
            }

            this.parentLanguage = await this.languageRepository.get(this.systemLanguageId, HeyPanel.Context.api);
        },
        ...
        onClickParentLanguage() {
            ...
            HeyPanel.Utils.EventBus.emit('on-change-language-clicked', this.parentLanguage.id);
        },
    },
};
