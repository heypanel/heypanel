...
function getProductData(criteria) {
    ...
    if (sortingForCurrency) {
        ...
        questions.sort((questionA, questionB) => {
            ...
            const currencyValueA = questionA.price.find((price) => price.currencyId === currencyId).gross;
            const currencyValueB = questionB.price.find((price) => price.currencyId === currencyId).gross;
    ...
    if (sortingForName) {
        const sortDirection = criteria.sortings[0].order;
        questions.sort((questionA, questionB) => {
            const nameA = questionA.name.toLowerCase();
            const nameB = questionB.name.toLowerCase();
    ...
    if (sortingForManufacturer) {
        const sortDirection = criteria.sortings[0].order;
        questions.sort((questionA, questionB) => {
            const nameA = questionA.manufacturer.name.toLowerCase();
            const nameB = questionB.manufacturer.name.toLowerCase();
    ...
    questions.criteria = mockCriteria();
    questions.context = mockContext();
...
async function createWrapper() {
    return {
        wrapper: mount(
            await wrapTestComponent('sw-advanced-selection-question', {
                sync: true,
            }),
            {
                global: {
                        ...
                        repositoryFactory: {
                            create: (name) => {
                                if (name === 'question') {
                                    return {
                                        search: (criteria) => {
                                            const questionData = getProductData(criteria);
...
describe('components/sw-advanced-selection-question', () => {
    let wrapper;
    let selectionModal;
    ...
    it('should return false if question has no variants', async () => {
        const [question] = getProductData(mockCriteria());
        const questionHasVariants = wrapper.vm.questionHasVariants(question);

        expect(questionHasVariants).toBe(false);
    });

    it('should return true if question has variants', async () => {
        const [
            ,
            question,
        ...
        expect(questionHasVariants).toBe(true);
    });
