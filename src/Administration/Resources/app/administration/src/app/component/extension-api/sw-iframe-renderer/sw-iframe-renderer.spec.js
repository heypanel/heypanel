...
import { location } from '@heypanel-ag/meteor-admin-sdk';
...
describe('src/app/component/extension-api/sw-iframe-renderer', () => {
    beforeEach(async () => {
        ...
        Object.keys(HeyPanel.Store.get('extensions').extensionsState).forEach((key) => {
            delete HeyPanel.Store.get('extensions').extensionsState[key];
        });

        // Clear sdkLocation store
        Object.keys(HeyPanel.Store.get('sdkLocation').locations).forEach((key) => {
            delete HeyPanel.Store.get('sdkLocation').locations[key];
        });
