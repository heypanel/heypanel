...
describe('src/app/component/form/sw-datepicker', () => {
    ...
    beforeEach(async () => {
        HeyPanel.Store.get('session').setCurrentUser({ timeZone: 'UTC' });
    });

    afterAll(() => {
        HeyPanel.Store.get('session').setCurrentUser(currentUser);
    });
    ...
    it('should use the admin locale', async () => {
        HeyPanel.Store.get('session').currentLocale = 'de-DE';
        wrapper = await createWrapper();
    ...
    it.each([
        ...
        async ({ dateType, timeZone, expectedTimeZone }) => {
            HeyPanel.Store.get('session').setCurrentUser({ timeZone: timeZone });
    ...
    it('should not convert the date when a timezone is set and dateType is date', async () => {
        HeyPanel.Store.get('session').setCurrentUser({ timeZone: 'Europe/Berlin' });
    ...
    it('should support other locales formats', async () => {
        HeyPanel.Store.get('session').currentLocale = 'en-US';
        wrapper = await createWrapper({});
        ...
        HeyPanel.Store.get('session').currentLocale = 'en-UK';
