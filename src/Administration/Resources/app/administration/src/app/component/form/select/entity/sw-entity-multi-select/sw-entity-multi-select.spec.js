...
describe('components/sw-entity-multi-select', () => {
    ...
    it('should not display variations', async () => {
        const swEntityMultiSelect = await createWrapper();
        const questionVariantInfo = swEntityMultiSelect.find('.sw-question-variant-info');

        expect(questionVariantInfo.exists()).toBe(false);
    });
    ...
    it('should display variations', async () => {
        ...
        const questionVariantInfo = wrapper.find('.sw-question-variant-info');
        expect(questionVariantInfo.exists()).toBe(true);

        expect(questionVariantInfo.find('.sw-question-variant-info__question-name').text()).toContain(fixture[0].name);
