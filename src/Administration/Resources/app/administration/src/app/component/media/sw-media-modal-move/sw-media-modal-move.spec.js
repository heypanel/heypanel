...
const createMediaEntity = (options = {}) => {
    return new Entity(HeyPanel.Utils.createId(), 'media', {
        fileName: 'test.png',
        ...options,
    });
};
...
const createFolderEntity = (options = {}) => {
    return new Entity(HeyPanel.Utils.createId(), 'media_folder', {
        name: 'test',
        parentId: null,
        ...options,
    });
...
describe('components/media/sw-media-modal-move', () => {
    ...
    it('handles fetchParentFolder Admin API error gracefully', async () => {
        ...
        await wrapper.vm.fetchParentFolder(HeyPanel.Utils.createId());
