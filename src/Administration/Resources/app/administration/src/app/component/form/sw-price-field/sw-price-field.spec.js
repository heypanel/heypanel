...
describe('components/form/sw-price-field', () => {
    beforeEach(() => {
        HeyPanel.Application.getContainer = () => {
            return {
                apiService: {
                    getByName() {
                        return {
    ...
    it('should cancel the debounce timer when the number field emits "ends-with-decimal-separator" event', async () => {
        ...
        await wrapper.findByPlaceholder('sw-question.priceForm.placeholderPriceGross').setValue('123');
        ...
        expect(wrapper.findByPlaceholder('sw-question.priceForm.placeholderPriceGross').element.value).toBe('123.');
    });
});
