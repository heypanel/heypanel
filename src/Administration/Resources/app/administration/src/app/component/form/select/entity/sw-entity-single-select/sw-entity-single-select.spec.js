...
describe('components/sw-entity-single-select', () => {
    ...
    it('should not display variations', async () => {
        const swEntitySingleSelect = await createEntitySingleSelect();
        await flushPromises();
        const questionVariantInfo = swEntitySingleSelect.find('.sw-question-variant-info');

        expect(questionVariantInfo.exists()).toBeFalsy();
    });
    ...
    it('should display variations', async () => {
        ...
        await swEntitySingleSelect.vm.$nextTick(() => {
            ...
            expect(questionVariantInfo.exists()).toBeTruthy();

            expect(questionVariantInfo.find('.sw-question-variant-info__question-name').text()).toEqual(fixture[0].name);

            expect(questionVariantInfo.find('.sw-question-variant-info__specification').text()).toContain(
    ...
    it('should recognize non-existing entity and offer entity creation', async () => {
        const nonExistingEntityMock = new EntityCollection('', '', HeyPanel.Context.api, null, [], 0);
                    ...
                    repositoryFactory: {
                        create: () => ({
                            search: (context) => {
                                ...
                                return Promise.resolve(new EntityCollection('', '', HeyPanel.Context.api, null, [], 0));
                            },
                            get: () =>
                                Promise.resolve({
                                    id: 'manufacturerId',
