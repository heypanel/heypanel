...
export default {
    ...
    computed: {
        notifications() {
            return Object.values(HeyPanel.Store.get('notification').growlNotifications);
        },
    ...
    methods: {
        onClose(notification) {
            HeyPanel.Store.get('notification').removeGrowlNotification(notification);
        },

        handleAction(action, notification) {
            // Allow external links for example to the heypanel account or store
            if (HeyPanel.Utils.string.isUrl(action.route)) {
