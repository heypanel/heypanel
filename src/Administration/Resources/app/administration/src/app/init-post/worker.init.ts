...
import type { ApiContext } from '@heypanel-ag/meteor-admin-sdk/es/_internals/data/EntityCollection';
...
export default function initializeWorker() {
    const loginService = HeyPanel.Service('loginService');
    const context = HeyPanel.Context.app;
    const workerNotificationFactory = HeyPanel.Application.getContainer('factory').workerNotification;
    const configService = HeyPanel.Service('configService');
        ...
        return configService.getConfig().then((response) => {
            Object.entries(response as ContextAppConfig).forEach(
                ([
                    ...
                    HeyPanel.Store.get('context').addAppConfigValue({
            ...
            enableWorkerNotificationListener(loginService, HeyPanel.Context.api);
            ...
            if (context.config.adminWorker?.enableAdminWorker && !enabled) {
                enableAdminWorker(loginService, HeyPanel.Context.api, context.config.adminWorker);
            }
        });
    }
...
function enableAdminWorker(
    ...
    const importExportService = HeyPanel.Service('importExport');
...
function registerThumbnailMiddleware(factory: typeof WorkerNotificationFactory) {
    ...
    factory.register('EsIndexingMessage', {
        name: 'HeyPanel\\Elasticsearch\\Framework\\Indexing\\IndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('generateThumbnailsMessage', {
        name: 'HeyPanel\\Core\\Content\\Media\\Message\\GenerateThumbnailsMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('PromotionIndexingMessage', {
        name: 'HeyPanel\\Core\\Checkout\\Promotion\\DataAbstractionLayer\\PromotionIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('ProductStreamIndexingMessage', {
        name: 'HeyPanel\\Core\\Content\\ProductStream\\DataAbstractionLayer\\ProductStreamIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'questionStream',
                ids,
                ...
                notification,
                {
                    title: 'global.notification-center.worker-listener.questionStream.title',
                    message: 'global.notification-center.worker-listener.questionStream.message',
                    success: 'global.notification-center.worker-listener.questionStream.messageSuccess',
                },
                50,
            );
        },
    ...
    factory.register('CategoryIndexingMessage', {
        name: 'HeyPanel\\Core\\Content\\Category\\DataAbstractionLayer\\CategoryIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'category',
                ids,
    ...
    factory.register('MediaIndexingMessage', {
        name: 'HeyPanel\\Core\\Content\\Media\\DataAbstractionLayer\\MediaIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'media',
                ids,
    ...
    factory.register('ChannelIndexingMessage', {
        name: 'HeyPanel\\Core\\System\\Channel\\DataAbstractionLayer\\ChannelIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'channel',
                ids,
    ...
    factory.register('RuleIndexingMessage', {
        name: 'HeyPanel\\Core\\Content\\Rule\\DataAbstractionLayer\\RuleIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'rule',
                ids,
    ...
    factory.register('ProductIndexingMessage', {
        name: 'HeyPanel\\Core\\Content\\Product\\DataAbstractionLayer\\ProductIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'question',
                ids,
                ...
                notification,
                {
                    title: 'global.notification-center.worker-listener.question.title',
                    message: 'global.notification-center.worker-listener.question.message',
                    success: 'global.notification-center.worker-listener.question.messageSuccess',
                },
                50,
            );
        },
    ...
    factory.register('ElasticSearchIndexingMessage', {
        name: 'HeyPanel\\Elasticsearch\\Framework\\Indexing\\ElasticsearchIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
            messageQueueNotification(
                'esIndexing',
                ids,
    ...
    factory.register('ImportExportMessage', {
        name: 'HeyPanel\\Core\\Content\\ImportExport\\Message\\ImportExportMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('FlowIndexingMessage', {
        name: 'HeyPanel\\Core\\Content\\Flow\\Indexing\\FlowIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
    ...
    factory.register('NewsletterRecipientIndexingMessage', {
        name: 'HeyPanel\\Core\\Content\\Newsletter\\DataAbstractionLayer\\NewsletterRecipientIndexingMessage',
        fn: function middleware(next, { entry, $root, notification }) {
