...
describe('src/app/init-post/user-information.init.ts', () => {
    ...
    beforeAll(() => {
        ...
        HeyPanel.Service().register('userService', () => {
            return {
    ...
    beforeEach(() => {
        HeyPanel.Store.get('session').setCurrentUser(undefined);
    ...
    it('should init the user context service correctly when user is logged in', async () => {
        ...
        expect(HeyPanel.Store.get('session').currentUser).toBeUndefined();

        await initializeUserContext();

        expect(initializeUserNotifications).toHaveBeenCalled();
        expect(HeyPanel.Store.get('session').currentUser).toEqual({
            username: 'my-fancy-username',
        });
    });
