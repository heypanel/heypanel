...
export default function initializeTabs(): void {
    HeyPanel.ExtensionAPI.handle('uiTabsAddTabItem', async (componentConfig) => {
        HeyPanel.Store.get('tabs').addTabItem(componentConfig);

        // Reload current route if it does not exist
        const router = HeyPanel.Application.view?.router as Router;
    ...
    void HeyPanel.Application.viewInitialized.then(() => {
        ...
        router.beforeResolve((to) => {
            ...
            // Get all tab routes
            const tabRoutes = Object.values(HeyPanel.Store.get('tabs').tabItems).reduce<string[]>((acc, tabItems) => {
                acc = [
                    ...acc,
            ...
            const moduleInfo = HeyPanel?.Application?.$container?.container?.init?.router?.getModuleInfo?.(parentRoute) as
                | {
            ...
            const newRoute: RouteRecordRaw = {
                path: newPath,
                component: HeyPanel.Application.view?.getComponent('sw-extension-component-section'),
                children: [],
                name: newRouteName,
                meta: {
