...
describe('src/app/init/settings-item.init.ts', () => {
    ...
    beforeEach(() => {
        HeyPanel.Store.get('extensionSdkModules').modules = [];
        HeyPanel.Store.get('settingsItems').settingsGroups = {
            shop: [],
            system: [],
            plugins: [],
        };
    ...
    it('should handle the settingsItemAdd requests', async () => {
        ...
        expect(HeyPanel.Store.get('extensionSdkModules').modules).toHaveLength(1);
        expect(HeyPanel.Store.get('extensionSdkModules').modules[0]).toEqual({
            baseUrl: '',
            displaySearchBar: true,
        ...
        expect(HeyPanel.Store.get('settingsItems').settingsGroups.system).toHaveLength(1);
        expect(HeyPanel.Store.get('settingsItems').settingsGroups.system[0]).toEqual({
            group: 'system',
            icon: 'default-object-books',
    ...
    it('should handle the settingsItemAdd requests with fallback', async () => {
        ...
        expect(HeyPanel.Store.get('settingsItems').settingsGroups.plugins).toHaveLength(1);
        expect(HeyPanel.Store.get('settingsItems').settingsGroups.plugins[0]).toEqual({
            group: 'plugins',
            icon: 'default-object-books',
            id: 'settings-location-id',
            label: 'App Settings',
    ...
    it('should not commit the extension when moduleID could not be generated', async () => {
        jest.spyOn(HeyPanel.Store.get('extensionSdkModules'), 'addModule').mockImplementationOnce(() => {
