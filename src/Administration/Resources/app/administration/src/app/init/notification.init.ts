...
export default function initializeNotifications(): void {
    ...
    HeyPanel.ExtensionAPI.handle('notificationDispatch', (notificationOptions) => {
        // @ts-expect-error - t is callable
        const message = notificationOptions.message ?? HeyPanel.Snippet.tc('global.notification.noMessage');
        // @ts-expect-error - tc is callable
        const title = notificationOptions.title ?? HeyPanel.Snippet.tc('global.notification.noTitle');
