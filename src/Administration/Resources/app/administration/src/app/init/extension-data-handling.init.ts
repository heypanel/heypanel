...
function getRepository(
    ...
    if (!extension) {
        throw new Error(
            ...
            `Could not find an extension with the given name "${extensionName}" in the extension store (HeyPanel.Store.get('extensions').extensionsState)`,
        );
    }

    if (extension.integrationId) {
        return HeyPanel.Service('repositoryFactory').create(entityName, '', {
...
export default function initializeExtensionDataLoader(): void {
    HeyPanel.ExtensionAPI.handle(
        'repositorySearch',
        async ({ entityName, criteria = new HeyPanel.Data.Criteria(), context }, additionalInformation) => {
            try {
                ...
                const mergedContext = { ...HeyPanel.Context.api, ...context };
    ...
    HeyPanel.ExtensionAPI.handle(
        'repositoryGet',
        ({ entityName, id, criteria = new HeyPanel.Data.Criteria(), context }, additionalInformation) => {
            const repository = getRepository(entityName as keyof EntitySchema.Entities, additionalInformation);
            if (!repository) {
    ...
    HeyPanel.ExtensionAPI.handle('repositorySave', ({ entityName, entity, context }, additionalInformation) => {
        const repository = getRepository(entityName as keyof EntitySchema.Entities, additionalInformation);
        if (!repository) {
    ...
    HeyPanel.ExtensionAPI.handle('repositoryClone', ({ entityName, behavior, entityId, context }, additionalInformation) => {
        const repository = getRepository(entityName as keyof EntitySchema.Entities, additionalInformation);
        if (!repository) {
    ...
    HeyPanel.ExtensionAPI.handle('repositoryHasChanges', ({ entityName, entity }, additionalInformation) => {
        const repository = getRepository(entityName as keyof EntitySchema.Entities, additionalInformation);
        if (!repository) {
    ...
    HeyPanel.ExtensionAPI.handle('repositorySaveAll', ({ entityName, entities, context }, additionalInformation) => {
        const repository = getRepository(entityName as keyof EntitySchema.Entities, additionalInformation);
        if (!repository) {
            return rejectRepositoryCreation(entityName as keyof EntitySchema.Entities) as Promise<void>;
        }
    ...
    HeyPanel.ExtensionAPI.handle('repositoryDelete', ({ entityName, entityId, context }, additionalInformation) => {
        const repository = getRepository(entityName as keyof EntitySchema.Entities, additionalInformation);
        if (!repository) {
            return rejectRepositoryCreation(entityName as keyof EntitySchema.Entities) as Promise<void>;
        }
    ...
    HeyPanel.ExtensionAPI.handle('repositoryCreate', ({ entityName, entityId, context }, additionalInformation) => {
        const repository = getRepository(entityName as keyof EntitySchema.Entities, additionalInformation);
        if (!repository) {
            return rejectRepositoryCreation(entityName as keyof EntitySchema.Entities) as Promise<
