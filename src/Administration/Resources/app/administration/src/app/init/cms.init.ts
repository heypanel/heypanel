...
export default function initializeCms(): void {
    HeyPanel.ExtensionAPI.handle('cmsRegisterElement', (element, additionalInformation) => {
    ...
    HeyPanel.ExtensionAPI.handle('cmsRegisterBlock', (block, additionalInformation) => {
        const extension = Object.values(HeyPanel.Store.get('extensions').extensionsState).find((ext) =>
            ext.baseUrl.startsWith(additionalInformation._event_.origin),
        );
        ...
        HeyPanel.Service('cmsService').registerCmsBlock({
            name: 'app-renderer',
            label: block.label ?? '',
