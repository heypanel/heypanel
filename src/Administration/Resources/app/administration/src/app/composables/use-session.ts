...
async function setAdminLocale(locale: string): Promise<void> {
    ...
    const localeToLanguageService = HeyPanel.Service('localeToLanguageService') as LocaleToLanguageService;
    return localeToLanguageService.localeToLanguage(locale).then((languageIdFromService) => {
        ...
        HeyPanel.Application.getContainer('factory').locale.storeCurrentLocale(locale);
    });
}
...
function setAdminLocaleState({
    ...
    if (!locales.find((l) => l === locale)) {
        HeyPanel.Utils.debug.warn('SessionStore', `Locale ${locale} not registered at store`);
        return;
    }
