...
import setupHeyPanelDevtools from 'src/app/adapter/view/sw-vue-devtools';
...
HeyPanel.Service().register('localeHelper', () => {
    return {
...
describe('ASYNC app/adapter/view/vue.adapter.js', () => {
    ...
    beforeEach(async () => {
        ...
        if (!HeyPanel.Service('loginService')) {
        ...
        if (!HeyPanel.Service('localeToLanguageService')) {
            HeyPanel.Service().register('localeToLanguageService', () => {
                return {
        ...
        HeyPanel.Store.get('system').locales = [
            'en-GB',
            'de-DE',
        ];

        HeyPanel.Store.get('session').setAdminLocaleState({
            locales: [
                'en-GB',
                'de-DE',
            ],
    ...
    it('initLocales should call setLocaleFromuser', async () => {
        ...
        HeyPanel.Store.get('session').currentLocale = 'en-GB';
    ...
    it('setLocaleFromUser should not set the user when user does not exist', async () => {
        ...
        expect(HeyPanel.Service('localeHelper').setLocaleWithId).not.toHaveBeenCalled();
    });

    it('setLocaleFromUser should set the user when user does not exist', async () => {
        HeyPanel.Store.get('session').setCurrentUser({ localeId: '12345' });
        vueAdapter.setLocaleFromUser();

        expect(HeyPanel.Service('localeHelper').setLocaleWithId).toHaveBeenCalled();
    });

    it('setLocaleFromUser should call the service with the user id from the store', async () => {
        const expectedId = '12345678';
        HeyPanel.Store.get('session').setCurrentUser({ localeId: expectedId });

        vueAdapter.setLocaleFromUser();

        expect(HeyPanel.Service('localeHelper').setLocaleWithId).toHaveBeenCalledWith(expectedId);
    });
    ...
    it('initLocales should watch for user changes and recall the "setLocaleWithId"', async () => {
        ...
        expect(HeyPanel.Service('localeHelper').setLocaleWithId).toHaveBeenCalledWith('german-id');
    });

    it('should resolve mixins by explicit Mixin get by name call', async () => {
        HeyPanel.Mixin.register('foo1', {
            methods: {
                fooBar() {
                    return this.title;
                },
        ...
        HeyPanel.Component.register('test-component1', {
            template: '<div></div>',
        ...
        HeyPanel.Component.markComponentAsSync('test-component1');
    ...
    it('should resolve mixins by explicit Mixin get by name call with override', async () => {
        ...
        HeyPanel.Component.register('test-component2', {
            template: '<div></div>',
        ...
        HeyPanel.Component.override('test-component2', {
            data() {
                return {
        ...
        HeyPanel.Component.markComponentAsSync('test-component2');
    ...
    it('should resolve mixins by string', async () => {
        ...
        HeyPanel.Component.register('test-component3', {
            template: '<div></div>',
        ...
        HeyPanel.Component.markComponentAsSync('test-component3');
    ...
    it('should resolve mixins by string with override', async () => {
        ...
        HeyPanel.Component.register('test-component4', {
            template: '<div></div>',
        ...
        HeyPanel.Component.override('test-component4', {
            data() {
                return {
        ...
        HeyPanel.Component.markComponentAsSync('test-component4');
    ...
    it('should resolve mixins for component in combination with overrides', async () => {
        HeyPanel.Mixin.register('foo-with-data', {
            data() {
                return {
                    sortBy: null,
                };
        ...
        HeyPanel.Component.register('test-component-foobar-with-mixin', {
            template: '<div></div>',
            name: 'test-component',
            data() {
                return {
        ...
        HeyPanel.Component.markComponentAsSync('test-component-foobar-with-mixin');
        ...
        HeyPanel.Component.override('test-component-foobar-with-mixin', {});
    ...
    it('should extend mixins', async () => {
        ...
        HeyPanel.Component.register('extendable-component', {
        ...
        HeyPanel.Component.extend('sw-test-component-extended', 'extendable-component', {
        ...
        HeyPanel.Component.markComponentAsSync('sw-test-component-extended');
    ...
    it('should allow multi-inheritance with multiple mixins and lifecycle hooks are only executed once', async () => {
        ...
        HeyPanel.Mixin.register('first-mixin', {
            created() {
                lifecycleSpy();
            },
            methods: {
        ...
        HeyPanel.Mixin.register('second-mixin', {
            methods: {
                bar() {
                    return 'bar';
                },
        ...
        HeyPanel.Component.register('base-component', {
            template: '<div class="base-component"></div>',
        });

        HeyPanel.Component.override('base-component', {
            mixins: ['first-mixin'],
        });
        ...
        HeyPanel.Component.markComponentAsSync('base-component');
    ...
    describe('should initialize everything correctly', () => {
        ...
        beforeEach(async () => {
            ...
            const locale = HeyPanel.Application.getContainer('factory').locale;
            if (!locale.getLocaleByName('en-GB')) {
                locale.register('en-GB', {
                    global: {
                        'sw-admin-menu': {
                            textHeyPanelAdmin: 'Text HeyPanel Admin',
                        },
                        my: {
                            mock: {
                                title: 'Mock title',
            ...
            if (!HeyPanel.Filter.getByName('my-mock-filter')) {
                HeyPanel.Filter.register('my-mock-filter', () => {
                    return 'mocked';
                });
            }

            if (!HeyPanel.Directive.getByName('my-mock-directive')) {
                HeyPanel.Directive.register('my-mock-directive', () => {
                    return {
                        bind() {},
                        inserted() {},
                        update() {},
            ...
            // add main component
            if (!HeyPanel.Component.getComponentRegistry().has('sw-admin')) {
                HeyPanel.Component.register('sw-admin', {
                    template: '<div class="sw-admin"></div>',
                });
            }

            // add VueAdapter to HeyPanel object
            HeyPanel.Application.setViewAdapter(vueAdapter);
        ...
        it('should have correct working createTitle method', () => {
            ...
            expect(result).toBe('Test | Mock title | Text HeyPanel Admin');
        });
        ...
        it('should setup the devtools in development environment', async () => {
            expect(setupHeyPanelDevtools).toHaveBeenCalled();
        });
