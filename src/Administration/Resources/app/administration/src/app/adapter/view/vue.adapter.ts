...
export default class VueAdapter extends ViewAdapter {
    ...
    initVue(renderElement: string, router: Router, providers: { [key: string]: unknown }): App<Element> {
        ...
        this.app.config.performance = process.env.NODE_ENV !== 'questionion';
         ...
         *
         * So we should convert from provide/inject to HeyPanel.Service
         */
        ...
        if (process.env.NODE_ENV === 'development') {
            setupHeyPanelDevtools(this.root);
        }
    ...
    async initDependencies() {
        ...
        const syncComponents = [
            ...
            'sw-settings-question-feature-sets-detail',
            'sw-system-config',
    ...
    async initComponents() {
        ...
        meteorComponents.forEach((componentName) => {
            const componentNameAsKebabCase = HeyPanel.Utils.string.kebabCase(componentName);
    ...
    initLocales() {
        ...
        const messages = {};
        const fallbackLocale = HeyPanel.Context.app.fallbackLocale as FallbackLocale;
        ...
        HeyPanel.Vue.watch(
            ...
            (newValue, oldValue) => {
                ...
                if (currentUserLocaleId && currentUserLocaleId !== oldUserLocaleId) {
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
                    HeyPanel.Service('localeHelper').setLocaleWithId(currentUserLocaleId);
                }
            },
            { deep: true },
        );
    ...
    setLocaleFromUser() {
        ...
        if (currentUser) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
            HeyPanel.Service('localeHelper').setLocaleWithId(userLocaleId);
        }
    }
    ...
    initTitle(app: App<Element>) {
        app.config.globalProperties.$createTitle = function createTitle(
            ...
            const baseTitle = this.$root.$tc('global.sw-admin-menu.textHeyPanelAdmin');
