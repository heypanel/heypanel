 ...
 * @sw-package checkout
 */
import type { iapCheckout } from '@heypanel-ag/meteor-admin-sdk/es/iap';
...
const inAppPurchaseCheckoutStore = HeyPanel.Store.register({
    ...
    actions: {
        request(entry: InAppPurchaseRequest, extension: string): void {
            if (!HeyPanel.Context.app.config.bundles?.[extension]) {
