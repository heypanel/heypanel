 ...
 */
import type HeyPanelError from 'src/core/data/HeyPanelError';
 ...
 * */
function setApiErrorNested({ expression, error, store }: { expression: string; error: HeyPanelError; store: ErrorStore }) {
...
function removeApiError(expression: string, store: ErrorStore) {
    ...
    if (HeyPanel.Utils.types.isEmpty(store)) {
 ...
 */
const errorStore = HeyPanel.Store.register({
    id: 'error',
    ...
    state: () =>
        ({
            ...
            system: Record<string, HeyPanelError>;
            api: Record<string, HeyPanelError>;
        },

    actions: {
        addApiError({ expression, error }: { expression: string; error: HeyPanelError }) {
        ...
        addSystemError({ error, id = utils.createId() }: { error: HeyPanelError; id?: string }) {
    ...
    getters: {
        getErrorsForEntity(state) {
            return (entityName: string, id: string) => {
                ...
                // @ts-expect-error
                return (entityStorage[id] as HeyPanelError) ?? null;
            };
        },
