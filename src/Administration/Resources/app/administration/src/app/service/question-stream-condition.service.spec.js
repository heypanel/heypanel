...
describe('app/service/question-stream-condition.service.js', () => {
    ...
    it('should be able to remove properties from entity allowlist', async () => {
        expect(service.isPropertyInAllowList('question', 'name')).toBe(true);
        service.removeFromEntityAllowList('question', ['name']);
        expect(service.isPropertyInAllowList('question', 'name')).toBe(false);
    });
