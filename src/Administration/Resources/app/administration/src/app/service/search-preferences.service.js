...
export default function SearchPreferencesService({ userConfigRepository: _userConfigRepository }) {
    ...
    function getDefaultSearchPreferences() {
        ...
        HeyPanel.Module.getModuleRegistry().forEach(({ manifest }) => {
            if (
                manifest.entity &&
                HeyPanel.Service('acl').can(`${manifest.entity}.editor`) &&
    ...
    function getUserSearchPreferences() {
        return new Promise((resolve) => {
            HeyPanel.Service('userConfigService')
    ...
    function processSearchPreferencesFields(tempSearchPreferencesFields) {
        ...
        tempSearchPreferencesFields.forEach((field) => {
            field.group.forEach((group) => {
                const searchPreferencesField = HeyPanel.Utils.object.set({}, group.fieldName, {
                    _searchable: field._searchable,
                    _score: field._score,
                });
                searchPreferencesFields = HeyPanel.Utils.object.deepMergeObject(
    ...
    function _getUserConfigCriteria() {
        const criteria = new HeyPanel.Data.Criteria();

        criteria.addFilter(HeyPanel.Data.Criteria.equals('key', KEY_USER_SEARCH_PREFERENCE));
        criteria.addFilter(HeyPanel.Data.Criteria.equals('userId', _getCurrentUser()?.id));
    ...
    function _getCurrentUser() {
        return HeyPanel.Store.get('session').currentUser;
    }
