...
export default class RuleConditionService {
    ...
    groups: { [key: string]: group } = {
        ...
        members: {
            id: 'member',
            name: 'sw-settings-rule.detail.groups.members',
        },
        promotions: {
            id: 'promotion',
    ...
    getRestrictedRules(entityName: string) {
        ...
        const ruleRepository = HeyPanel.Service('repositoryFactory').create('rule');
    ...
    getTranslatedConditionViolationList(violations: Array<{ label: string }>, connectionSnippetPath: string) {
        const app = HeyPanel.Application.getApplicationRoot();
        if (!app) {
            return '';
        }
