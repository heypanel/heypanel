const { Application, Store } = HeyPanel;
...
export default function createLicenseViolationsService(storeService) {
    ...
    async function forceDeletePlugin(extension) {
        const heypanelExtensionService = HeyPanel.Service('heypanelExtensionService');
        const cacheService = HeyPanel.Service('cacheApiService');
        ...
        try {
            ...
            if (isActive) {
                await heypanelExtensionService.deactivateExtension(extension.name, extension.type);
                await cacheService.clear();
            }

            if (isInstalled) {
                await heypanelExtensionService.uninstallExtension(extension.name, extension.type);
            }

            await heypanelExtensionService.removeExtension(extension.name, extension.type);
