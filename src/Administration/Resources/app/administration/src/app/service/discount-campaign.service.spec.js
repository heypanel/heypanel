...
describe('src/app/service/discount-campaign.service.ts', () => {
    ...
    const heypanelDiscountCampaignService = new HeyPanelDiscountCampaignService();
    ...
    describe('isDiscountCampaignActive', () => {
        it('returns true if today is between startDate and endDate and no duration is set', async () => {
            ...
            expect(heypanelDiscountCampaignService.isDiscountCampaignActive(campaign)).toBe(true);
        });
        ...
        it('returns false if discountCampaign is null', async () => {
            expect(heypanelDiscountCampaignService.isDiscountCampaignActive(null)).toBe(false);
        });

        it('returns false if discountCampaign has no startDate', async () => {
            expect(
                heypanelDiscountCampaignService.isDiscountCampaignActive({
                    startDate: null,
                }),
            ).toBe(false);
        });
        ...
        it('returns false if startDate is in the future', async () => {
            ...
            expect(heypanelDiscountCampaignService.isDiscountCampaignActive(campaign)).toBe(false);
        });
    ...
    describe('isSamePeriod', () => {
        ...
        it('returns true if startDate, endDate and discountAppliesForMonths are the same', async () => {
            expect(heypanelDiscountCampaignService.isSamePeriod(originalCampaign, originalCampaign)).toBe(true);
        });

        it.each(differences)('returns false if %s is different', (propName, differentCampaign) => {
            expect(heypanelDiscountCampaignService.isSamePeriod(originalCampaign, differentCampaign)).toBe(false);
        });
    });
});
