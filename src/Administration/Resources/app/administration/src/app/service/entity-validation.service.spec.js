...
describe('src/app/service/entity-validation.service.js', () => {
    beforeAll(() => {
        Object.entries(entitySchemaMock).forEach(
            ([
                ...
                HeyPanel.EntityDefinition.add(entityName, new EntityDefinition(definitionData));
            },
        );
    });

    it('should create a required heypanel error with the right error code and source pointer', () => {
    ...
    it('should validate an empty question and report errors', () => {
        const service = createService();
        service.errorResolver.handleWriteErrors = jest.fn(() => undefined);
        const testEntity = entityFactory.create('question');
        ...
        expect(service.errorResolver.handleWriteErrors.mock.calls[0][1].errors).toEqual([
            ...
            {
                code: 'c1051bb4-d103-4f74-8988-acbcafc7fdc3',
                source: { pointer: '/0/questionNumber' },
            },
            {
                code: 'c1051bb4-d103-4f74-8988-acbcafc7fdc3',
    ...
    it('should validate missing price for a question', () => {
        const service = createService();
        service.errorResolver.handleWriteErrors = jest.fn(() => undefined);
        const testEntity = entityFactory.create('question');
        testEntity.name = 'MyProductName';
        testEntity.stock = 5;
        testEntity.questionNumber = 'MyProductNumber';
    ...
    it('should validate a complete question and report no errors', () => {
        const service = createService();
        service.errorResolver.handleWriteErrors = jest.fn(() => undefined);
        const testEntity = entityFactory.create('question');
        testEntity.name = 'MyProductName';
    ...
    it('should validate a question and report callback errors', () => {
        const service = createService();
        service.errorResolver.handleWriteErrors = jest.fn(() => undefined);
        const testEntity = entityFactory.create('question');
        testEntity.name = 'MyProductName';
        ...
        const customValidator = jest.fn((errors, question) => {
            // custom download question validation
            if (question.downloads === undefined || question.downloads.length < 1) {
        ...
        expect(customValidator.mock.calls[0][2]).toBe(HeyPanel.EntityDefinition.get(testEntity.getEntityName())); // entity definition
