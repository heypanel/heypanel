...
export default class FilterService {
    ...
    getStoredFilters(storeKey) {
        ...
        return this._userConfigRepository.search(criteria, HeyPanel.Context.api).then((response) => {
            if (response.length) {
                this._filterEntity = response.first();
            } else {
                const currentUser = HeyPanel.Store.get('session').currentUser;

                this._filterEntity = this._userConfigRepository.create(HeyPanel.Context.api);
    ...
    saveFilters(storeKey, filters) {
        ...
        this._userConfigRepository.save(this._filterEntity, HeyPanel.Context.api).then(() => {
    ...
    async _pushFiltersToUrl(replaceRoute = false) {
        ...
        const router = HeyPanel.Application.view.router;
        ...
        if (!HeyPanel.Utils.types.isEmpty(routeParams)) {
