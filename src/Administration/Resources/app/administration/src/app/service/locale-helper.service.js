...
export default class LocaleHelperService {
    ...
    constructor({ HeyPanel, localeRepository, snippetService, localeFactory }) {
        this._HeyPanel = HeyPanel;
    ...
    async setLocaleWithId(localeId) {
        const { code } = await this._localeRepository.get(localeId, this._HeyPanel.Context.api);
    ...
    async setLocaleWithCode(localeCode) {
        ...
        await this._HeyPanel.Store.get('session').setAdminLocale(localeCode);
    }
}
