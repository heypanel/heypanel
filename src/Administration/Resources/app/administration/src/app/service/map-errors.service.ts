...
export function mapPropertyErrors<T extends string, K extends string>(
    ...
    properties.forEach((property) => {
        const computedValueName = HeyPanel.Utils.string.camelCase(`${entityName}.${property}.error`);
        ...
        computedValues[computedValueName] = function getterPropertyError() {
            ...
            return HeyPanel.Store.get('error').getApiError(entity, property);
        };
    });
...
export function mapSystemConfigErrors(entityName: string, saleChannelId: string | null, key: string = ''): $TSFixMe {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
    return HeyPanel.Store.get('error').getSystemConfigApiError(entityName, saleChannelId!, key);
}
...
export function mapCollectionPropertyErrors<T extends string, K extends string>(
    ...
    properties.forEach((property) => {
        const computedValueName = HeyPanel.Utils.string.camelCase(`${entityCollectionName}.${property}.error`);
        ...
        computedValues[computedValueName] = function getterCollectionError() {
            ...
            return entityCollection.map((entity) => HeyPanel.Store.get('error').getApiError(entity, property));
        };
    });
...
export function mapPageErrors<T extends string>(
    ...
    Object.keys(errorConfig).forEach((routeName) => {
        ...
        map[`${HeyPanel.Utils.string.camelCase(routeName)}Error`] = function getterPropertyError() {
            return Object.keys(subjects).some((entityName) => {
                // eslint-disable-next-line @typescript-eslint/no-unsafe-call, @typescript-eslint/no-unsafe-member-access
                return HeyPanel.Store.get('error').existsErrorInProperty(entityName, subjects[entityName]);
            });
        };
    });
    return map;
