...
import type { MessageDataType, HandleMethod, BaseMessageOptions } from '@heypanel-ag/meteor-admin-sdk/es/channel';
import { handle as sdkHandle } from '@heypanel-ag/meteor-admin-sdk/es/channel';
import type { HeyPanelMessageTypes } from '@heypanel-ag/meteor-admin-sdk/es/message-types';
...
export default {
    ...
    handle: <MESSAGE_TYPE extends keyof HeyPanelMessageTypes>(
        type: MESSAGE_TYPE,
        ...
        const aclHook = (
            ...
            return new Promise((resolve, reject) => {
                ...
                const missingPrivileges = data.privileges!.filter((p) => !HeyPanel.Service('acl').can(p));
                if (missingPrivileges.length > 0) {
                    reject(new MissingPrivilegesError(type, missingPrivileges));
                } else {
                    const result = method(data, additionalInformation);

                    if (isPromise<HeyPanelMessageTypes[MESSAGE_TYPE]['responseType']>(result)) {
