...
describe('core/application.js', () => {
    const originalInjectJs = HeyPanel.Application.injectJs;
    const originalInjectIframe = HeyPanel.Application.injectIframe;
    const originalNodeEnv = process.env.NODE_ENV;

    beforeEach(() => {
        jest.clearAllMocks();
        HeyPanel.Application.injectJs = originalInjectJs;
        HeyPanel.Application.injectIframe = originalInjectIframe;
    ...
    it("should be error tolerant if loading a plugin's files fails", async () => {
        ...
        HeyPanel.Application.injectJs = async () => {
            throw new Error('Inject js fails');
        };

        const result = await HeyPanel.Application.injectPlugin({
            js: ['some.js'],
        });
    ...
    it('should call swagCommercial before any other plugins', async () => {
        ...
        HeyPanel.Application.injectPlugin = async (plugin) => {
            callOrder.js.push(plugin.js);
            callOrder.css.push(plugin.css);
        };

        // load all plugins
        await HeyPanel.Application.loadPlugins();
    ...
    it('should load plugins correctly in prod', async () => {
        ...
        HeyPanel.Application.injectIframe = jest.fn();
        ...
        expect(HeyPanel.Application.injectIframe).toHaveBeenCalledWith({
