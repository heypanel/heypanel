...
class ApplicationBootstrapper {
     ...
     *
     * @example
     * HeyPanel.Application.addFactory('module', (container) => {
     ...
     *
     * @example
     * HeyPanel.Application.addFactoryMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * HeyPanel.Application.addFactoryMiddleware('module', (service, next) => {
     ...
     *
     * @example
     * HeyPanel.Application.addFactoryDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * HeyPanel.Application.addFactoryDecorator('module', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
     ...
     *
     * @example
     * HeyPanel.Application.addInitializer('httpClient', (container) => {
     ...
     *
     * @example
     * HeyPanel.Application.addServiceProvider('questionService', (container) => {
    ...
    registerApiContext(context: ContextStore['api']): ApplicationBootstrapper {
        HeyPanel.Context.api = HeyPanel.Classes._private.ApiContextFactory(context);
    ...
    registerAppContext(context: ContextStore['app']): ApplicationBootstrapper {
        HeyPanel.Context.app = HeyPanel.Classes._private.AppContextFactory(context);
     ...
     *
     * @example
     * HeyPanel.Application.addServiceProviderMiddleware((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * HeyPanel.Application.addServiceProviderMiddleware('questionService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    initializeFeatureFlags(): ApplicationBootstrapper {
        const features = HeyPanel.Context.app.features;
        ...
        HeyPanel.Feature.init(features);
     ...
     *
     * @example
     * HeyPanel.Application.addServiceProviderDecorator((container, next) => {
     *    // Do something with the container
     *    next();
     * });
     *
     * @example
     * HeyPanel.Application.addServiceProviderDecorator('questionService', (service, next) => {
     *    // Do something with the service
     *    next();
     * });
     */
    ...
    bootFullApplication(): Promise<void | ApplicationBootstrapper> {
        ...
        return this.initializeInitializers(initPreContainer, '-pre')
            ...
            .then(() => Promise.all(HeyPanel.Plugin.getBootPromises()))
            .then(() => {
                if (!this.view) {
                    return Promise.reject();
                }
    ...
    createApplicationRoot(): Promise<ApplicationBootstrapper> {
        ...
        if (HeyPanel.Context.app.environment === 'testing') {
        ...
        const firstRunWizard = HeyPanel.Context.app.firstRunWizard;
    ...
    private async loadPlugins(): Promise<(unknown[] | null)[]> {
        ...
        if (isDevelopmentMode) {
            ...
            if (HeyPanel.Utils.object.hasOwnProperty(plugins, 'metadata')) {
                delete plugins.metadata;
            }
        } else {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            plugins = HeyPanel.Context.app.config.bundles as bundlesPluginResponse;
        }

        if (plugins['hey-cart']) {
            await this.injectPlugin(plugins['hey-cart']);
        }
        ...
        const injectAllPlugins = Object.entries(plugins)
            .filter(([pluginName]) => {
                // Filter the hey-cart bundle because it was loaded beforehand
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
        const bundles = HeyPanel.Context.app.config.bundles as bundlesPluginResponse;
                ...
                if (isDevelopmentMode) {
                    ...
                    Object.entries(plugins).forEach(
                        ([
                            ...
                            const stringUtils = HeyPanel.Utils.string;
    ...
    private injectIframe({
        ...
        const bundles = HeyPanel.Context.app.config.bundles;
        ...
        HeyPanel.Store.get('extensions').addExtension(extension);
    }
}
