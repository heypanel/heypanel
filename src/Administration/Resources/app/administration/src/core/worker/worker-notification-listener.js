...
const { Application, WorkerNotification } = HeyPanel;
...
class WorkerNotificationListener {
    ...
    start() {
        if (!HeyPanel.Context.app.config.adminWorker.enableQueueStatsWorker) {
            return;
        }
    ...
    setupIntervalWatcher() {
        ...
        HeyPanel.Vue.watch(
            HeyPanel.Store.get('notification').workerProcessPollInterval,
    ...
    _checkQueue() {
        ...
        const client = HeyPanel.Application.getContainer('init').httpClient;
        const headers = {
            headers: {
                Authorization: `Bearer ${HeyPanel.Service('loginService').getToken()}`,
            },
        };
        ...
        client.get('/_info/queue.json', headers).then((res) => {
            ...
            if (this._isRunning) {
                this._interval = HeyPanel.Store.get('notification').workerProcessPollInterval;
                this._timeoutId = setTimeout(this._checkQueue.bind(this), this._interval);
            }
        });
    }
    ...
    runNotificationMiddleware(response) {
        ...
        const middlewareParams = {
            ...
            notification: {
                create: (notification) => {
                    return HeyPanel.Store.get('notification').createNotification(notification);
                },
                update: (notification) => {
                    return HeyPanel.Store.get('notification').updateNotification(notification);
                },
            },
            queue,
        };
