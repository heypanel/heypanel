...
export default class Repository<EntityName extends keyof EntitySchema.Entities> {
    ...
    get schema(): EntityDefinition<any> {
        return HeyPanel.EntityDefinition.get(this.entityName);
    }

    /**
     * Sends a search request to the server to find entity ids for the provided criteria.
     */
    searchIds(criteria: Criteria, context = HeyPanel.Context.api): Promise<IdSearchResult> {
     ...
     */
    search(criteria: Criteria, context = HeyPanel.Context.api): Promise<EntityCollection<EntityName>> {
        const headers = this.buildHeaders(context);
     ...
     */
    get(id: string, context = HeyPanel.Context.api, criteria: Criteria | null = null): Promise<Entity<EntityName> | null> {
     ...
     */
    save(entity: Entity<EntityName>, context = HeyPanel.Context.api): Promise<void | AxiosResponse> {
     ...
     */
    clone(entityId: string, behavior: $TSDangerUnknownObject, context = HeyPanel.Context.api): Promise<unknown> {
        if (!entityId) {
     ...
     */
    saveAll(entities: EntityCollection<EntityName>, context = HeyPanel.Context.api): Promise<unknown> {
     ...
     * @private
     */
    sendUpserts(changeset: $TSDangerUnknownObject[], failOnError: boolean, context = HeyPanel.Context.api): Promise<void> {
     ...
     * where the base route contains already the owner key, e.g. /question/{id}/categories
     * The provided id contains the associated entity id.
     */
    assign(id: string, context = HeyPanel.Context.api): Promise<AxiosResponse> {
        const headers = this.buildHeaders(context);
     ...
     */
    delete(id: string, context = HeyPanel.Context.api): Promise<AxiosResponse> {
        const headers = this.buildHeaders(context);
     ...
     */
    syncDeleted(ids: string[], context = HeyPanel.Context.api): Promise<void> {
        const headers = this.buildHeaders(context);
     ...
     */
    create(context = HeyPanel.Context.api, id: string | null = null): Entity<EntityName> {
     ...
     */
    mergeVersion(versionId: string, context = HeyPanel.Context.api): Promise<AxiosResponse> {
        const headers = this.buildHeaders(context);
     ...
     */
    deleteVersion(entityId: string, versionId: string, context = HeyPanel.Context.api): Promise<AxiosResponse> {
        const headers = this.buildHeaders(context);
     ...
     */
    sendDeletions(queue: DeletionQueue, context = HeyPanel.Context.api): Promise<AxiosResponse[]> {
        const headers = this.buildHeaders(context);
     ...
     */
    buildHeaders(context = HeyPanel.Context.api): {
        Accept: string;
        ...
        const { hasOwnProperty } = HeyPanel.Utils.object;
