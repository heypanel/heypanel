...
describe('src/core/data/error-resolver.data', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(HeyPanel.Store.get('error'), 'resetApiErrors');
        jest.spyOn(HeyPanel.Store.get('error'), 'addApiError');
        jest.spyOn(HeyPanel.Store.get('error'), 'addSystemError');
    ...
    describe('resetApiErrors', () => {
        it('should dispatches "error/resetApiErrors" action', () => {
            ...
            expect(HeyPanel.Store.get('error').resetApiErrors).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe('handleWriteErrors', () => {
        ...
        it('should handles write errors and adds system errors', () => {
            ...
            const changeset = [
                {
                    entity: entityFactory.create('member'),
                    changes: [
                        {
                            firstName: 'a',
                            lastName: 'b',
            ...
            expect(HeyPanel.Store.get('error').addApiError).toHaveBeenCalledTimes(2);
            expect(HeyPanel.Store.get('error').addSystemError).toHaveBeenCalledTimes(1);
            expect(HeyPanel.Store.get('error').addApiError).toHaveBeenNthCalledWith(1, {
                expression: expect.anything(),
                error: expect.any(HeyPanel.Classes.HeyPanelError),
            });
            expect(HeyPanel.Store.get('error').addApiError).toHaveBeenNthCalledWith(2, {
                expression: expect.anything(),
                error: expect.any(HeyPanel.Classes.HeyPanelError),
            });
            expect(HeyPanel.Store.get('error').addSystemError).toHaveBeenCalledWith(
                expect.any(HeyPanel.Classes.HeyPanelError),
            );
        });

        it('should convert to HeyPanelError', () => {
            const errors = [
                { source: { pointer: '/0/firstName' }, code: 'CODE1' },
            ];
            ...
            expect(HeyPanel.Store.get('error').addApiError).toHaveBeenCalledWith({
                expression: expect.anything(),
                error: expect.any(HeyPanel.Classes.HeyPanelError),
            });
        });
    ...
    describe('getErrorPath', () => {
        it('should returns the correct error path', () => {
            const entity = {
                getEntityName: jest.fn(() => 'question'),
                id: 'abc123',
            };
            ...
            expect(result).toBe('question.abc123.name');
        });
    });
    ...
    describe('handleDeleteError', () => {
        it('should handle delete errors and add system errors and api errors', () => {
            ...
            expect(HeyPanel.Store.get('error').addSystemError).toHaveBeenCalledTimes(2);
            expect(HeyPanel.Store.get('error').addApiError).toHaveBeenCalledTimes(2);
            expect(HeyPanel.Store.get('error').addSystemError).toHaveBeenCalledWith({
                error: expect.any(HeyPanel.Classes.HeyPanelError),
            });
