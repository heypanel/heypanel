...
export default function createLoginService(
    ...
    function verifyUserToken(password: string): Promise<string> {
        ...
        return verifyUserByUsername(HeyPanel.Store.get('session').currentUser?.username ?? '', password)
            .then(({ access }) => {
                if (HeyPanel.Utils.types.isString(access)) {
                    return access;
                }
    ...
    function loginByUsername(user: string, pass: string): Promise<AuthObject> {
        return httpClient
            ...
            .then((response) => {
                HeyPanel.Service('userActivityService').updateLastUserActivity();
    ...
    function lastActivityOverThreshold(): boolean {
        const lastActivity = HeyPanel.Service('userActivityService').getLastUserActivity().getTime();
    ...
    function shouldConsiderUserActivity(): boolean {
        ...
        const devEnv = HeyPanel.Context.app.environment === 'development';
    ...
    function forwardLogout(isInactivityLogout: boolean, shouldRedirect: boolean): void {
        ...
        const router = HeyPanel.Application.view.router as null | Router;
        if (router) {
            const id = HeyPanel.Utils.createId();
            ...
            if (isInactivityLogout && shouldRedirect) {
                ...
                void html2canvas(document.querySelector('#app'), {
                    ...
                    sessionStorage.setItem('lastKnownUser', HeyPanel.Store.get('session').currentUser?.username ?? '');
