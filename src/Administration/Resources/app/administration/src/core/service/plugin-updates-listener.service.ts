...
export default function addPluginUpdatesListener(loginService: LoginService, serviceContainer: ServiceContainer): void {
    ...
    function createUpdatesAvailableNotification(): void {
        const root = HeyPanel.Application.getApplicationRoot();
        ...
        void HeyPanel.Store.get('notification').createNotification(notification);
    }
    ...
    HeyPanel.Vue.watch(useSession().currentUser, (newValue) => {
        if (!newValue) {
            return;
        }
