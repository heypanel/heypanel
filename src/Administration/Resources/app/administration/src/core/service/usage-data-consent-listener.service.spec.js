...
describe('src/core/service/usage-data-consent-listener.service.ts', () => {
    ...
    beforeEach(() => {
        ...
        loginService = createLoginService({}, HeyPanel.Context.api);
        serviceContainer = {
            usageDataService: {
                getConsent: () => {
        ...
        HeyPanel.Store.get('usageData').updateConsent({
    ...
    it('should update consent on login', async () => {
        ...
        expect(HeyPanel.Store.get('usageData').isConsentGiven).toBe(true);
        expect(HeyPanel.Store.get('usageData').isBannerHidden).toBe(false);
    });
    ...
    it('should reset the consent if the request fails', async () => {
        ...
        expect(HeyPanel.Store.get('usageData').isConsentGiven).toBe(false);
        expect(HeyPanel.Store.get('usageData').isBannerHidden).toBe(true);
    });
