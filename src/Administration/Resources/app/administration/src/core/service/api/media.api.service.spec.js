...
function getMediaApiService(client = null, loginService = null) {
    ...
    if (loginService === null) {
        loginService = createLoginService(client, HeyPanel.Context.api);
    }
...
describe('storeService', () => {
    ...
    it('test getDefaultFolderId without result', async () => {
        ...
        const spyRepository = jest.spyOn(HeyPanel.Service('repositoryFactory'), 'create').mockImplementation(() => {
            return {
                search: async () => {
        ...
        expect(await mediaApiService.getDefaultFolderId('question_download')).toBeNull();
    ...
    it('test getDefaultFolderId with folder', async () => {
        ...
        expect(await mediaApiService.getDefaultFolderId('question_download')).toBe('question_download_id');
        expect(await mediaApiService.getDefaultFolderId('question_download')).toBe('question_download_id');
        expect(mediaApiService.cacheDefaultFolder).toMatchObject({
            question_download: 'question_download_id',
        });
