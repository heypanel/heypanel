...
describe('src/core/service/utils/format.utils.js', () => {
    ...
    describe('date', () => {
        const setLocale = (locale) => {
            jest.spyOn(HeyPanel.Application.getContainer('factory').locale, 'getLastKnownLocale').mockImplementation(
                () => locale,
            );
        };
        const setTimeZone = (timeZone) => HeyPanel.Store.get('session').setCurrentUser({ timeZone });
    ...
    describe('currency', () => {
        const currencyFilter = HeyPanel.Utils.format.currency;
        ...
        it('should fallback to decimal when the currency ISO code is invalid', async () => {
            HeyPanel.Context.app.systemCurrencyISOCode = 'INVALID_EXAMPLE_CURRENCY_CODE';
