...
describe('src/core/service/plugin-update-listener.service.ts', () => {
    ...
    it('should not update the key if the notification could not be shown', async () => {
        ...
        jest.spyOn(HeyPanel.Application, 'getApplicationRoot').mockImplementation(() => {
            return false;
        });
    ...
    it('should update the key and show a notification', async () => {
        ...
        jest.spyOn(HeyPanel.Utils, 'createId').mockImplementation(() => 'jest');
        ...
        const notifications = HeyPanel.Store.get('notification');
