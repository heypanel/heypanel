...
import { updateSubscriber, register, handleGet } from '@heypanel-ag/meteor-admin-sdk/es/data';
import { get, debounce, cloneDeepWith } from 'lodash';
import type { App } from 'vue';
import { selectData } from '@heypanel-ag/meteor-admin-sdk/es/_internals/data/selectData';
import MissingPrivilegesError from '@heypanel-ag/meteor-admin-sdk/es/_internals/privileges/missing-privileges-error';
...
handleGet((data, additionalOptions) => {
    ...
    if (registeredDataSet.deprecated) {
        ...
        // @ts-expect-error
        if (process.env !== 'prod') {
            HeyPanel.Utils.debug.error(...debugArgs);
        } else {
            HeyPanel.Utils.debug.warn(...debugArgs);
        }
    }
    ...
    updateSubscriber(id, (value) => {
        ...
        function setObject(transferObject: transferObject, prePath: string | null = null): void {
            ...
            Object.keys(transferObject).forEach((property) => {
                ...
                if (
                    // @ts-expect-error
                    HeyPanel.Utils.hasOwnProperty(transferObject[property], 'getDraft', this) &&
                    // @ts-expect-error
                    typeof transferObject[property].getDraft === 'function'
                ) {
                    setObject(
                        {
                            [property]: HeyPanel.Utils.object.cloneDeep(transferObject[property]),
                        },
                        realPath,
                    );
                ...
                HeyPanel.Utils.object.get(scope, parsedPath.pathToLastSegment)[parsedPath.lastSegment] =
        ...
        if (path.includes('.')) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            HeyPanel.Utils.object.get(scope, newPath)[lastPath] = value.data;
