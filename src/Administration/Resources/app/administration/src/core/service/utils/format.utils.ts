...
export function currency(val: number, sign: string, decimalPlaces: number, additionalOptions: CurrencyOptions = {}): string {
    ...
    const opts = {
        style: 'currency',
        currency: sign || (HeyPanel.Context.app.systemCurrencyISOCode as string),
        ...decimalOpts,
        ...additionalOptions,
    };
    ...
    try {
        result = val.toLocaleString(
            additionalOptions.language ?? HeyPanel.Store.get('session').currentLocale ?? 'en-US',
...
export function date(val: string, options: DateFilterOptions = {}): string {
    ...
    const userTimeZone = HeyPanel?.Store?.get('session')?.currentUser?.timeZone ?? 'UTC';
