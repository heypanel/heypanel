...
function createClient() {
    const client = Axios.create({
        baseURL: HeyPanel.Context.api.apiPath,
    });
    ...
    client.interceptors.response.use(
        ...
        (error) => {
            ...
            try {
                ...
                HeyPanel.Utils.debug.error(e);

                if (errors) {
                    errors.forEach((singleError) => {
                        HeyPanel.Store.get('notification').createNotification({
                            variant: 'error',
...
/**
 * Determines the different status codes and creates a matching error via HeyPanel.State
 * @param {Number} status
 * @param {Array} errors
 * @param {Object} error
 * @param {Object} data
...
function handleErrorStates({ status, errors, error = null, data }) {
    ...
    const viewRoot = HeyPanel.Application.view.root;
    ...
    if (status === 403) {
        ...
        missingPrivilegeErrors.forEach((missingPrivilegeError) => {
            ...
            HeyPanel.Store.get('notification').createNotification({
                ...
                autoClose: false,
                growl: true,
                title: HeyPanel.Snippet.tc('global.error-codes.FRAMEWORK__MISSING_PRIVILEGE_ERROR'),
                message: `${HeyPanel.Snippet.tc('sw-privileges.error.description')} <br> ${missingPrivilegesMessage}`,
            });
        });
    }
    ...
    if (
        ...
        HeyPanel.Store.get('notification').createNotification({
            ...
            autoClose: false,
            growl: true,
            title: HeyPanel.Snippet.tc('sw-extension.errors.storeSessionExpired.title'),
            message: HeyPanel.Snippet.tc('sw-extension.errors.storeSessionExpired.message'),
            actions: [
                {
                    label: HeyPanel.Snippet.tc('sw-extension.errors.storeSessionExpired.actionLabel'),
                    method: () => {
    ...
    if (status === 409) {
        if (errors[0].code === 'FRAMEWORK__DELETE_RESTRICTED') {
            ...
            blockingEntities = parameters.usages.reduce((message, usageObject) => {
                ...
                const timesSnippet = HeyPanel.Snippet.tc('global.default.xTimesIn', times);
                const blockingEntitiesSnippet = HeyPanel.Snippet.tc(`global.entities.${usageObject.entityName}`, times[1]);
                return `${message}<br>${timesSnippet} <b>${blockingEntitiesSnippet}</b>`;
            }, '');

            HeyPanel.Store.get('notification').createNotification({
                variant: 'error',
                title: HeyPanel.Snippet.tc('global.default.error'),
                message: `${HeyPanel.Snippet.tc('global.notification.messageDeleteFailed', 3, {
                    entityName: HeyPanel.Snippet.tc(`global.entities.${entityName}`),
                })}${blockingEntities}`,
            });
        }
    }
            ...
            if (status === 401) {
                ...
                return new Promise((resolve, reject) => {
                    tokenHandler.subscribe(
                        ...
                        (err) => {
                            if (!HeyPanel.Application.getApplicationRoot()) {
                                reject(err);
...
function tracingInterceptor(client) {
    ...
    if (process.env.NODE_ENV !== 'test') {
        client.interceptors.request.use((config) => {
            const currentRoute = HeyPanel?.Application?.view?.router?.currentRoute?.value?.name;

            if (currentRoute) {
                config.headers['heypanel-admin-active-route'] = currentRoute;
            }
